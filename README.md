...
| 🚨 Accelerated Recovery Mode | Emergency recovery path with partner tool integration (ControlMonkey, N2W, Firefly). The result panel shows the partner-tool steps **plus three AWS Oversight Rails** the customer is responsible for — Health pre-flight, IAM least-privilege scoping for the tool's role, and post-recovery validation via AWS Backup Restore Testing + Resilience Hub. Customer accountability vs partner-tool action are visually separated. |
...
| 🤝 Partner Matchmaking & Regional Partner | Weighted scoring engine recommends the best-fit AWS partner. Both partner paths now show a six-step **Partner Engagement & Validation** runbook framed as customer-side oversight — define RTO/RPO from REL13-BP01, AWS Health pre-flight, scope partner IAM access with `sts:ExternalId`, demand a measured RTO via an end-to-end restore drill (AWS Backup Restore Testing, AWS Elastic Disaster Recovery drill instances, native-snapshot restore, or the customer's own backup tool — whichever fits the customer's posture), run a Resilience Hub assessment on the result, then revoke access and audit CloudTrail. Every step quotes AWS docs verbatim (REL13-BP01, REL13-BP03, Shared Responsibility Model for Resiliency, AWS Backup Restore Testing, Resilience Hub overview, IAM third-party-access guide). |
