...
(function () {
  ...
  var RULES_ENGINE = {
    ...
    getRunbookSteps: function (s) {
      ...
      steps.push({
        ...
        workarounds: [
          {
            title: 'Plan non-overlapping CIDRs in IPAM before creating any subnets',
            pattern: 'Amazon VPC IPAM — CIDR planning for cross-region migration',
            summary: 'Per AWS: "Amazon VPC IP Address Manager (IPAM) is a VPC feature that makes it easier for you to plan, track, and monitor IP addresses for your AWS workloads." IPAM can "Organize IP address space into routing and security domains," "Automatically allocate CIDRs to VPCs using specific business rules," and "Enable cross-region and cross-account sharing of your Bring Your Own IP (BYOIP) addresses." Overlapping CIDRs between source and target regions break TGW peering, VPC peering, and any future workload mesh — so this is the first decision to lock down. Use IPAM to allocate the target-region VPC from a reserved pool that does not overlap with the source-region VPC, on-prem ranges, or any other workload domain.',
            command: '# 1. Create an IPAM (run from the management/operations account):\naws ec2 create-ipam \\\n  --description "Multi-region migration IPAM" \\\n  --operating-regions RegionName=<SOURCE_REGION> RegionName=<TARGET_REGION>\n\n# 2. Create a top scope and per-region pools:\naws ec2 create-ipam-pool \\\n  --ipam-scope-id <SCOPE_ID> \\\n  --address-family ipv4 \\\n  --provisioned-cidrs Cidr=10.0.0.0/8 \\\n  --description "Top-level enterprise pool"\n\naws ec2 create-ipam-pool \\\n  --ipam-scope-id <SCOPE_ID> \\\n  --source-ipam-pool-id <TOP_POOL_ID> \\\n  --address-family ipv4 \\\n  --locale <TARGET_REGION> \\\n  --description "Target-region pool — non-overlapping with source-region pool"\n\n# 3. Allocate the target VPC from this pool, NOT a free-form CIDR:\naws ec2 create-vpc \\\n  --ipv4-ipam-pool-id <TARGET_POOL_ID> \\\n  --ipv4-netmask-length 16 \\\n  --region <TARGET_REGION>',
            sources: [
              { label: 'Amazon VPC IPAM overview', url: 'https://docs.aws.amazon.com/vpc/latest/ipam/what-it-is-ipam.html' }
            ]
          },
          {
            title: 'Span subnets across at least 2 AZs in the target region from day one',
            pattern: 'AWS Builders\' Library — Static stability + multi-AZ',
            summary: 'Per AWS: "Each subnet must reside entirely within one Availability Zone and cannot span zones. By launching AWS resources in separate Availability Zones, you can protect your applications from the failure of a single Availability Zone." Per the AWS Builders\' Library on static stability, the recovery posture must include enough capacity in each remaining AZ to absorb traffic if one AZ fails — pre-stage the subnets, the security groups, and any pre-warmed compute capacity at provisioning time, not at recovery time. AWS-recommended posture: 2 AZs minimum (3 if the workload uses quorum-based services like Aurora multi-AZ, ElastiCache, or self-managed Kafka).',
            command: '# Create public + private subnets in EVERY AZ from the AWS-recommended set:\nfor AZ in <AZ1> <AZ2> <AZ3>; do\n  aws ec2 create-subnet \\\n    --vpc-id <TARGET_VPC_ID> \\\n    --cidr-block <CIDR_PER_AZ> \\\n    --availability-zone $AZ \\\n    --tag-specifications "ResourceType=subnet,Tags=[{Key=Tier,Value=public},{Key=AZ,Value=$AZ}]" \\\n    --region <TARGET_REGION>\ndone\n\n# Confirm all expected AZs are covered:\naws ec2 describe-subnets \\\n  --filters Name=vpc-id,Values=<TARGET_VPC_ID> \\\n  --query "Subnets[].{AZ:AvailabilityZone,CIDR:CidrBlock,Tier:Tags[?Key==\\`Tier\\`].Value | [0]}" \\\n  --region <TARGET_REGION>',
            sources: [
              { label: 'VPC Subnets — AZ requirement', url: 'https://docs.aws.amazon.com/vpc/latest/userguide/configure-subnets.html' },
              { label: 'Builders\' Library — Static Stability', url: 'https://aws.amazon.com/builders-library/static-stability-using-availability-zones/' }
            ]
          },
          {
            title: 'Enable VPC Flow Logs at provisioning time so post-migration validation has data to query',
            pattern: 'VPC Flow Logs — DR validation',
            summary: 'Per AWS: "VPC Flow Logs is a feature that enables you to capture information about the IP traffic going to and from network interfaces in your VPC. Flow log data can be published to the following locations: Amazon CloudWatch Logs, Amazon S3, or Amazon Data Firehose." And: "Flow logs can help you with a number of tasks, such as: Diagnosing overly restrictive security group rules; Monitoring the traffic that is reaching your instance; Determining the direction of the traffic to and from the network interfaces." Important: "Flow log data is collected outside of the path of your network traffic, and therefore does not affect network throughput or latency." Enable flow logs immediately on the new VPC — without them, post-cutover incident triage has no traffic data to query.',
            command: '# Create an S3 bucket for the flow logs (low cost, queryable from Athena):\naws s3api create-bucket \\\n  --bucket vpc-flowlogs-<ACCT_ID>-<TARGET_REGION> \\\n  --region <TARGET_REGION> \\\n  --create-bucket-configuration LocationConstraint=<TARGET_REGION>\n\n# Enable flow logs at the VPC level capturing ALL traffic:\naws ec2 create-flow-logs \\\n  --resource-type VPC \\\n  --resource-ids <TARGET_VPC_ID> \\\n  --traffic-type ALL \\\n  --log-destination-type s3 \\\n  --log-destination arn:aws:s3:::vpc-flowlogs-<ACCT_ID>-<TARGET_REGION> \\\n  --max-aggregation-interval 60 \\\n  --region <TARGET_REGION>',
            sources: [
              { label: 'VPC Flow Logs', url: 'https://docs.aws.amazon.com/vpc/latest/userguide/flow-logs.html' }
            ]
          },
          {
            title: 'Pre-create VPC Reachability Analyzer paths so cutover validation is one CLI call',
            pattern: 'VPC Reachability Analyzer — Pre-staged DR validation',
            summary: 'Per AWS: "Reachability Analyzer is a configuration analysis tool that enables you to perform connectivity testing between a source resource and a destination resource in your virtual private clouds (VPCs). When the destination is reachable, Reachability Analyzer produces hop-by-hop details of the virtual network path between the source and the destination. When the destination is not reachable, Reachability Analyzer identifies the blocking component. For example, paths can be blocked by configuration issues in a security group, network ACL, route table, or load balancer." Use cases per AWS: "Verify that your network configuration matches your intended connectivity" and "Automate the verification of your connectivity intent as your network configuration changes." Pre-stage the analysis paths (e.g., bastion → DB, ALB → app, app → SQS endpoint) at VPC provisioning so a single re-run after cutover validates the entire reachability surface.',
            command: '# 1. Create a Network Insights path (template for repeated analyses):\naws ec2 create-network-insights-path \\\n  --source <APP_SUBNET_ENI_ID> \\\n  --destination <DB_INSTANCE_ENI_ID> \\\n  --protocol tcp \\\n  --destination-port 5432 \\\n  --region <TARGET_REGION>\n\n# 2. Run an analysis on the path; rerun this any time SG/NACL/route changes:\naws ec2 start-network-insights-analysis \\\n  --network-insights-path-id <PATH_ID> \\\n  --region <TARGET_REGION>\n\n# 3. Inspect the result:\naws ec2 describe-network-insights-analyses \\\n  --network-insights-analysis-ids <ANALYSIS_ID> \\\n  --query "NetworkInsightsAnalyses[].{Status:Status,Reachable:NetworkPathFound,Blocker:Explanations}" \\\n  --region <TARGET_REGION>',
            sources: [
              { label: 'VPC Reachability Analyzer', url: 'https://docs.aws.amazon.com/vpc/latest/reachability/what-is-reachability-analyzer.html' }
            ]
          }
        ],
        refs: [
          ...
          { label: 'VPC Subnets', url: 'https://docs.aws.amazon.com/vpc/latest/userguide/configure-subnets.html' },
          { label: 'VPC IPAM', url: 'https://docs.aws.amazon.com/vpc/latest/ipam/what-it-is-ipam.html' },
          { label: 'VPC Flow Logs', url: 'https://docs.aws.amazon.com/vpc/latest/userguide/flow-logs.html' },
          { label: 'VPC Reachability Analyzer', url: 'https://docs.aws.amazon.com/vpc/latest/reachability/what-is-reachability-analyzer.html' },
      ...
      if (s.networkConnectivity === 'direct-connect') {
        steps.push({
          ...
          description: 'Provision DX connection, virtual interfaces, and DX Gateway. Note: Direct Connect provisioning involves physical infrastructure and has significant lead times. Plan accordingly. Per AWS: the Direct Connect Resiliency Toolkit offers Maximum Resiliency, High Resiliency, and Development/Test models — the right model depends on whether the workload needs protection against device, connectivity, or location failures.',
          workarounds: [
            {
              ...
              command: '# Inspect existing DX connections to see what posture you have today:\naws directconnect describe-connections --output table\n\n# Order a second connection in a DIFFERENT location (Maximum Resiliency model):\naws directconnect create-connection \\\n  --location <SECOND_DX_LOCATION_CODE> \\\n  --bandwidth 1Gbps \\\n  --connection-name migration-dx-secondary \\\n  --region <TARGET_REGION>\n\n# Pair the two connections via a LAG (link aggregation group) for unified routing:\naws directconnect create-lag \\\n  --number-of-connections 2 \\\n  --location <DX_LOCATION_CODE> \\\n  --connections-bandwidth 1Gbps \\\n  --lag-name migration-dx-lag \\\n  --region <TARGET_REGION>',
              sources: [
            ...
            {
              title: 'Stand up a Site-to-Site VPN as a backup path while DX provisions (and keep it permanently)',
              pattern: 'AWS Direct Connect + VPN — Backup path',
              summary: 'Per AWS: "Direct Connect provisioning involves physical infrastructure and has significant lead times" — typically weeks to months. AWS-recommended: provision a Site-to-Site VPN to the same VGW or DX Gateway during the wait, and keep it as a permanent BGP-routed backup path. AWS prefers DX-advertised BGP routes over VPN-advertised routes by default for the same prefix, so the failover from DX to VPN happens automatically when DX BGP drops. This converts a DX outage from a hard failure into a degraded-bandwidth event. Also useful when DX requires planned maintenance.',
              command: '# Create the VPN to the same VGW that DX terminates on:\naws ec2 create-customer-gateway \\\n  --type ipsec.1 --public-ip <ON_PREM_PUBLIC_IP> --bgp-asn 65000 \\\n  --region <TARGET_REGION>\n\naws ec2 create-vpn-connection \\\n  --type ipsec.1 \\\n  --customer-gateway-id <CGW_ID> \\\n  --vpn-gateway-id <SAME_VGW_AS_DX> \\\n  --options "{\\"StaticRoutesOnly\\":false}" \\\n  --region <TARGET_REGION>\n\n# Verify both paths are up; DX should win the BGP best-path selection:\naws directconnect describe-virtual-interfaces --region <TARGET_REGION>\naws ec2 describe-vpn-connections --region <TARGET_REGION> \\\n  --query "VpnConnections[].{State:State,VgwTunnels:VgwTelemetry[].Status}"',
              sources: [
                { label: 'AWS Direct Connect — Resiliency', url: 'https://docs.aws.amazon.com/directconnect/latest/UserGuide/disaster-recovery-resiliency.html' },
                { label: 'AWS Site-to-Site VPN — Redundant connections', url: 'https://docs.aws.amazon.com/vpn/latest/s2svpn/vpn-redundant-connection.html' }
              ]
            }
          ],
          refs: [
        ...
        steps.push({
          ...
          description: 'Create TGW peering between source and target regions. Per AWS, transit gateway route tables enable separation of routing domains: "Use transit gateway route tables to configure routing for your transit gateway attachments... Additionally you can create and manage static routes with a route table. For example, you might have a static route that\'s used as a backup route in the event of a network disruption that affects any dynamic routes."',
          workarounds: [
            {
              title: 'Use separate TGW route tables to isolate prod vs DR vs shared-services',
              pattern: 'AWS Transit Gateway — Route table separation',
              summary: 'Per AWS: "A route table is a table that contains rules that direct how your network traffic is routed between your VPCs and VPNs. Each route in the table contains the range of IP addresses for the destinations that you want to send traffic to." And: "Transit gateway route tables allows you to associate a table with a transit gateway attachment. VPC, VPN, VPN Concentrator, Client VPN, Direct Connect gateway, Peering, and Connect attachments are all supported. When associated, routes for these attachments are propagated from the attachment to the target transit gateway route table. An attachment can be propagated to multiple route tables." Best practice for DR: keep a dedicated route table per environment (prod/dr/shared) and propagate only the attachments that environment is allowed to reach. This prevents accidental cross-environment routing during cutover.',
              command: '# Create dedicated route tables in the target region:\naws ec2 create-transit-gateway-route-table \\\n  --transit-gateway-id <TARGET_TGW_ID> \\\n  --tag-specifications "ResourceType=transit-gateway-route-table,Tags=[{Key=Name,Value=prod}]" \\\n  --region <TARGET_REGION>\naws ec2 create-transit-gateway-route-table \\\n  --transit-gateway-id <TARGET_TGW_ID> \\\n  --tag-specifications "ResourceType=transit-gateway-route-table,Tags=[{Key=Name,Value=dr}]" \\\n  --region <TARGET_REGION>\n\n# Associate the cross-region peering attachment with the DR route table only:\naws ec2 associate-transit-gateway-route-table \\\n  --transit-gateway-attachment-id <PEERING_ATTACHMENT_ID> \\\n  --transit-gateway-route-table-id <DR_ROUTE_TABLE_ID> \\\n  --region <TARGET_REGION>\n\n# Add a static backup route per AWS guidance:\naws ec2 create-transit-gateway-route \\\n  --destination-cidr-block <SOURCE_CIDR> \\\n  --transit-gateway-route-table-id <DR_ROUTE_TABLE_ID> \\\n  --transit-gateway-attachment-id <PEERING_ATTACHMENT_ID> \\\n  --region <TARGET_REGION>',
              sources: [
                { label: 'AWS Transit Gateway — Route tables', url: 'https://docs.aws.amazon.com/vpc/latest/tgw/tgw-route-tables.html' }
              ]
            },
            {
              title: 'Verify TGW route propagation per attachment, not just at the gateway level',
              pattern: 'TGW — Route propagation pre-flight',
              summary: 'Per AWS: TGW route tables can have multiple attachments propagating routes into them. The single most common DR-day surprise is a peering attachment that exists but has not propagated routes into the DR route table. Verify both the association AND the propagation before trusting cross-region traffic. Use TGW Network Manager (or the export-route-tables-to-S3 feature) to capture a baseline.',
              command: '# Verify the peering attachment is associated with the expected route table:\naws ec2 get-transit-gateway-route-table-associations \\\n  --transit-gateway-route-table-id <DR_ROUTE_TABLE_ID> \\\n  --region <TARGET_REGION>\n\n# Verify routes have actually propagated into the route table:\naws ec2 get-transit-gateway-route-table-propagations \\\n  --transit-gateway-route-table-id <DR_ROUTE_TABLE_ID> \\\n  --region <TARGET_REGION>\n\n# Run the actual route lookup that the data plane will use:\naws ec2 search-transit-gateway-routes \\\n  --transit-gateway-route-table-id <DR_ROUTE_TABLE_ID> \\\n  --filters Name=route-search.exact-match,Values=<SOURCE_CIDR> \\\n  --region <TARGET_REGION>\n\n# Optional: export the route tables to S3 for diff-against-known-good baselines:\naws ec2 export-transit-gateway-routes \\\n  --transit-gateway-route-table-id <DR_ROUTE_TABLE_ID> \\\n  --s3-bucket <BACKUP_BUCKET> \\\n  --region <TARGET_REGION>',
              sources: [
                { label: 'AWS Transit Gateway — Route tables', url: 'https://docs.aws.amazon.com/vpc/latest/tgw/tgw-route-tables.html' }
              ]
            }
          ...
          refs: [
            { label: 'AWS Transit Gateway — Route tables', url: 'https://docs.aws.amazon.com/vpc/latest/tgw/tgw-route-tables.html' },
            { label: 'AWS Transit Gateway — Inter-region peering', url: 'https://docs.aws.amazon.com/vpc/latest/tgw/tgw-peering.html' }
          ],
          commands: [
        ...
        steps.push({
          ...
          description: 'Create VPN connection to target region. Per AWS: every Site-to-Site VPN connection ships with two tunnels for HA — but tunnel-level redundancy alone is not customer-gateway redundancy. AWS recommends a second customer gateway device to protect against on-premises hardware failures, with BGP advertising the same routes from both devices.',
          workarounds: [
            {
              title: 'Add a second customer gateway device — AWS-recommended for VPN HA',
              pattern: 'AWS Site-to-Site VPN — Redundant customer gateways',
              summary: 'Per AWS: "To protect against a loss of connectivity in case your customer gateway device becomes unavailable, you can set up a second Site-to-Site VPN connection to your VPC and virtual private gateway by adding a second customer gateway device. By using redundant VPN connections and customer gateway devices, you can perform maintenance on one of your devices while traffic continues to flow over the second VPN connection." And: "Configure a second customer gateway device. Both devices should advertise the same IP ranges to the virtual private gateway. We use BGP routing to determine the path for traffic. If one customer gateway device fails, the virtual private gateway directs all traffic to the working customer gateway device." Statically routed VPN does not provide this — AWS explicitly recommends BGP: "We recommend that you configure your network to use the routing information provided by BGP (if available) to select an available path."',
              command: '# Provision a second customer gateway in the same target region:\naws ec2 create-customer-gateway \\\n  --type ipsec.1 \\\n  --public-ip <ON_PREM_DEVICE_2_PUBLIC_IP> \\\n  --bgp-asn 65000 \\\n  --region <TARGET_REGION>\n\n# Attach a second VPN connection to the SAME VGW with BGP:\naws ec2 create-vpn-connection \\\n  --type ipsec.1 \\\n  --customer-gateway-id <CGW_2_ID> \\\n  --vpn-gateway-id <SAME_VGW_ID> \\\n  --options \'{"StaticRoutesOnly":false}\' \\\n  --region <TARGET_REGION>\n\n# Verify both connections show 2 tunnels UP each (4 tunnels total across the redundant pair):\naws ec2 describe-vpn-connections --region <TARGET_REGION> \\\n  --query "VpnConnections[].{Id:VpnConnectionId,Tunnels:VgwTelemetry[].Status}"',
              sources: [
            ...
            {
              title: 'Use BGP-routed VPN with active-active tunnels — never static',
              pattern: 'Site-to-Site VPN — Dynamic routing for failover',
              summary: 'Per AWS: "Dynamically routed Site-to-Site VPN connections use the Border Gateway Protocol (BGP) to exchange routing information between your customer gateways and the virtual private gateways. Statically routed Site-to-Site VPN connections require you to enter static routes for the remote network on your side of the customer gateway. BGP-advertised and statically entered route information allow gateways on both sides to determine which tunnels are available and reroute traffic if a failure occurs. We recommend that you configure your network to use the routing information provided by BGP (if available) to select an available path." Static routing forces manual operator intervention to fail over — exactly the kind of step that fails during real incidents. Always BGP for DR-grade VPN.',
              command: '# Inspect each VPN connection to confirm StaticRoutesOnly is FALSE:\naws ec2 describe-vpn-connections --region <TARGET_REGION> \\\n  --query "VpnConnections[].{Id:VpnConnectionId,Static:Options.StaticRoutesOnly,Routing:Routes[].Source}"\n\n# If a VPN was created statically by accident, recreate it with BGP routing.\n# AWS does not support converting static-to-dynamic on an existing VPN connection.',
              sources: [
                { label: 'Site-to-Site VPN — Redundant connections (BGP recommendation)', url: 'https://docs.aws.amazon.com/vpn/latest/s2svpn/vpn-redundant-connection.html' }
              ]
            }
          ],
          refs: [
      ...
      if (s.networkSecurity === 'security-groups' || s.networkSecurity === 'both') {
        steps.push({
          ...
          description: 'Capture all security group configurations from the source region, recreate them in the target region with matching inbound/outbound rules, and validate that rules are correctly applied. Critical AWS-documented constraint: "You can\'t reference the security group of a peer VPC that\'s in a different Region. Instead, use the CIDR block of the peer VPC." Any source-region SG rule that references another SG by ID will fail to translate to the target region — those rules must be rewritten as CIDR rules.',
          workarounds: [
            {
              title: 'Translate cross-VPC SG-ID references to CIDR rules — AWS does not support cross-region SG references',
              pattern: 'AWS VPC peering — Cross-region SG limitation',
              summary: 'Per AWS (verbatim): "You can\'t reference the security group of a peer VPC that\'s in a different Region. Instead, use the CIDR block of the peer VPC." This means every source-region SG that references another SG by ID (e.g., "allow port 5432 from sg-app-tier") must be rewritten when migrating across regions. The replacement is the CIDR of the source/destination subnet. AWS also documents stale-rule risk: "A stale security group rule is a rule that references a deleted security group in the same VPC or in a peer VPC, or that references a security group in a peer VPC for which the VPC peering connection has been deleted. When a security group rule becomes stale, it\'s not automatically removed." Identify these BEFORE cutover so the migration does not silently lose connectivity.',
              command: '# Identify source-region SGs that use SG-ID references (UserIdGroupPairs):\naws ec2 describe-security-groups --region <SOURCE_REGION> \\\n  --query "SecurityGroups[].{ID:GroupId,Name:GroupName,InboundRefs:IpPermissions[?length(UserIdGroupPairs)>\\`0\\`].UserIdGroupPairs}" \\\n  --output json > sg-with-references.json\n\n# Build the cross-region translation: (source SG-ID -> source subnet CIDR)\naws ec2 describe-security-groups --region <SOURCE_REGION> \\\n  --query "SecurityGroups[].{ID:GroupId,VpcId:VpcId}" --output json > sg-vpc-map.json\naws ec2 describe-subnets --region <SOURCE_REGION> \\\n  --query "Subnets[].{Vpc:VpcId,CIDR:CidrBlock,AZ:AvailabilityZone}" --output json > subnet-cidrs.json\n\n# In the target region, rewrite each SG-ID rule as a CIDR rule.\n# Example: replace "allow tcp 5432 from sg-app-tier" with "allow tcp 5432 from 10.10.0.0/24":\naws ec2 authorize-security-group-ingress \\\n  --group-id <TARGET_DB_SG_ID> \\\n  --protocol tcp --port 5432 \\\n  --cidr 10.10.0.0/24 \\\n  --region <TARGET_REGION>\n\n# Detect stale rules in the source region BEFORE the migration (clean up first):\naws ec2 describe-stale-security-groups \\\n  --vpc-id <SOURCE_VPC_ID> \\\n  --region <SOURCE_REGION>',
              sources: [
                { label: 'VPC peering — Cross-region SG limitation', url: 'https://docs.aws.amazon.com/vpc/latest/peering/vpc-peering-security-groups.html' }
              ]
            },
            {
              title: 'Detect SG drift between source and target with AWS Config conformance packs',
              pattern: 'AWS Config — Continuous SG drift detection',
              summary: 'Per the VPC SG best-practices: "Authorize only specific IAM principals to create and modify security groups. Create the minimum number of security groups that you need, to decrease the risk of error. Use each security group to manage access to resources that have similar functions and security requirements." Once you migrate, the source-region SGs continue to evolve and the target-region replicas drift. AWS Config conformance packs can flag SG drift, overly permissive rules (e.g., 0.0.0.0/0 on SSH/RDP per the same AWS best practice), and unattached SGs in either region. Use AWS Config rules `restricted-ssh`, `restricted-common-ports`, and `vpc-sg-open-only-to-authorized-ports` as the baseline.',
              command: '# Enable AWS Config in the target region (recorder + delivery channel):\naws configservice put-configuration-recorder \\\n  --configuration-recorder name=default,roleARN=<CONFIG_ROLE_ARN>,recordingGroup=allSupported=true \\\n  --region <TARGET_REGION>\naws configservice start-configuration-recorder --configuration-recorder-name default --region <TARGET_REGION>\n\n# Deploy the Operational Best Practices conformance pack (templates published by AWS):\naws configservice put-conformance-pack \\\n  --conformance-pack-name VPC-SecurityGroup-Baseline \\\n  --template-s3-uri s3://aws-config-conformance-packs/Operational-Best-Practices-for-Amazon-VPC.yaml \\\n  --region <TARGET_REGION>\n\n# Inspect SG findings:\naws configservice describe-compliance-by-config-rule \\\n  --config-rule-names restricted-ssh restricted-common-ports vpc-sg-open-only-to-authorized-ports \\\n  --region <TARGET_REGION>',
              sources: [
                { label: 'VPC Security Groups — Best practices', url: 'https://docs.aws.amazon.com/vpc/latest/userguide/vpc-security-groups.html' }
              ]
            },
            {
              title: 'Use AWS Firewall Manager security-group policies for org-wide enforcement post-migration',
              pattern: 'AWS Firewall Manager — Security group policy',
              summary: 'Per AWS Firewall Manager docs: "AWS Firewall Manager simplifies your administration and maintenance tasks across multiple accounts and resources for a variety of protections, including AWS WAF, AWS Shield Advanced, Amazon VPC security groups and network ACLs, AWS Network Firewall, and Amazon Route 53 Resolver DNS Firewall." Specifically: it "Allows you to apply security group rules to all member accounts or specific subsets of accounts in an AWS Organizations organization, and automatically applies the rules to new in-scope accounts that join the organization." This means after the migration, both regions can be governed by the same baseline SG policy enforced from the management account — drift between regions becomes impossible by construction, not by audit.',
              command: '# Designate the Firewall Manager admin account (run from Organizations management account):\naws fms associate-admin-account --admin-account <ADMIN_ACCOUNT_ID>\n\n# Push a baseline SG policy that applies to ALL VPCs in target region:\naws fms put-policy \\\n  --policy file://fms-baseline-sg-policy.json \\\n  --region <TARGET_REGION>\n\n# fms-baseline-sg-policy.json shape (abbreviated):\n# {\n#   "Policy": {\n#     "PolicyName": "baseline-sg",\n#     "ResourceType": "AWS::EC2::SecurityGroup",\n#     "RemediationEnabled": true,\n#     "SecurityServicePolicyData": {\n#       "Type": "SECURITY_GROUPS_COMMON",\n#       "ManagedServiceData": "..."\n#     }\n#   }\n# }',
              sources: [
          ...
          refs: [
            { label: 'VPC Security Groups overview', url: 'https://docs.aws.amazon.com/vpc/latest/userguide/vpc-security-groups.html' },
            { label: 'VPC peering — Cross-region SG limitation', url: 'https://docs.aws.amazon.com/vpc/latest/peering/vpc-peering-security-groups.html' },
            { label: 'AWS Firewall Manager', url: 'https://docs.aws.amazon.com/waf/latest/developerguide/fms-chapter.html' }
          ],
          commands: [
      ...
      if (s.networkSecurity === 'nacls' || s.networkSecurity === 'both') {
        steps.push({
          ...
          description: 'Capture all NACL configurations from the source region, recreate them in the target region with matching rules and rule numbers, and associate with the correct subnets. Per AWS: NACLs are stateless — "If, for example, you create a NACL rule to allow specific inbound traffic to a subnet, responses to that traffic are not automatically allowed." This is the most common NACL bug after migration: outbound rules are forgotten, or the ephemeral-port range for return traffic is not opened.',
          workarounds: [
            {
              title: 'Open the AWS-documented ephemeral port range for return traffic in BOTH directions',
              pattern: 'Stateless NACL — Ephemeral port handling',
              summary: 'Per AWS: "NACLs are stateless, which means that information about previously sent or received traffic is not saved. If, for example, you create a NACL rule to allow specific inbound traffic to a subnet, responses to that traffic are not automatically allowed. This is in contrast to how security groups work. Security groups are stateful." For every "allow inbound port X" rule there must be a corresponding "allow outbound ephemeral ports" rule (and vice versa). Different OSes use different ephemeral ranges (Linux 32768-60999, Windows 49152-65535). Most commonly the range is opened to 1024-65535 to cover both. Skipping this is the #1 reason NACL-protected applications work in dev (default-allow NACL) but fail after migration to a custom NACL.',
              command: '# Inbound for client → server (HTTPS):\naws ec2 create-network-acl-entry \\\n  --network-acl-id <TARGET_NACL_ID> --rule-number 100 \\\n  --protocol tcp --port-range From=443,To=443 \\\n  --cidr-block 0.0.0.0/0 --rule-action allow --ingress \\\n  --region <TARGET_REGION>\n\n# Outbound ephemeral range for return traffic — required because NACL is stateless:\naws ec2 create-network-acl-entry \\\n  --network-acl-id <TARGET_NACL_ID> --rule-number 100 \\\n  --protocol tcp --port-range From=1024,To=65535 \\\n  --cidr-block 0.0.0.0/0 --rule-action allow --egress \\\n  --region <TARGET_REGION>\n\n# Inbound ephemeral range to receive responses for outbound flows initiated from the subnet:\naws ec2 create-network-acl-entry \\\n  --network-acl-id <TARGET_NACL_ID> --rule-number 110 \\\n  --protocol tcp --port-range From=1024,To=65535 \\\n  --cidr-block 0.0.0.0/0 --rule-action allow --ingress \\\n  --region <TARGET_REGION>',
              sources: [
                { label: 'VPC Network ACLs — Stateless behavior', url: 'https://docs.aws.amazon.com/vpc/latest/userguide/vpc-network-acls.html' }
              ]
            },
            {
              title: 'Number rules in increments (10 or 100) so future rules can be inserted between them',
              pattern: 'NACL — Rule numbering for evolution',
              summary: 'Per AWS: "Each rule has a number from 1 to 32766. We evaluate the rules in order, starting with the lowest numbered rule, when deciding whether allow or deny traffic. If the traffic matches a rule, the rule is applied and we do not evaluate any additional rules. We recommend that you start by creating rules in increments (for example, increments of 10 or 100) so that you can insert new rules later on, if needed." When migrating, do NOT preserve the source-region rule numbers verbatim if they are sequential (1,2,3,…) — that traps you the first time a new exception is needed and forces a renumber across all dependent automation.',
              command: '# Create rules in increments of 100, leaving room between them:\naws ec2 create-network-acl-entry --network-acl-id <NACL_ID> --rule-number 100 \\\n  --protocol tcp --port-range From=443,To=443 --cidr-block 0.0.0.0/0 --rule-action allow --ingress --region <TARGET_REGION>\naws ec2 create-network-acl-entry --network-acl-id <NACL_ID> --rule-number 200 \\\n  --protocol tcp --port-range From=22,To=22 --cidr-block 10.0.0.0/8 --rule-action allow --ingress --region <TARGET_REGION>\naws ec2 create-network-acl-entry --network-acl-id <NACL_ID> --rule-number 300 \\\n  --protocol tcp --port-range From=1024,To=65535 --cidr-block 0.0.0.0/0 --rule-action allow --ingress --region <TARGET_REGION>\n\n# Audit current numbering to confirm space exists for inserts:\naws ec2 describe-network-acls --network-acl-ids <NACL_ID> --region <TARGET_REGION> \\\n  --query "NetworkAcls[].Entries[?Egress==\\`false\\`].{Num:RuleNumber,Action:RuleAction,Port:PortRange,CIDR:CidrBlock}"',
              sources: [
                { label: 'VPC Network ACLs — Rule numbering', url: 'https://docs.aws.amazon.com/vpc/latest/userguide/vpc-network-acls.html' }
              ]
            },
            {
              title: 'Use NACLs for defense-in-depth on top of SGs — they are not a substitute',
              pattern: 'AWS-recommended — NACL + SG layered',
              summary: 'Per AWS Security Groups documentation: "Consider creating network ACLs with rules similar to your security groups, to add an additional layer of security to your VPC." And per VPC subnet security guidance: "In most cases, security groups can meet your needs. However, you can use network ACLs if you want an additional layer of security." Important AWS-documented limitations of NACLs: they "can\'t block DNS requests to or from the Route 53 Resolver" and "can\'t block traffic to the Instance Metadata Service (IMDS)." Plan SGs for application-level access decisions and NACLs for blast-radius containment (e.g., subnet-wide deny of known-bad CIDRs).',
              command: '# Validate the NACL + SG combination delivers the intended reachability\n# using VPC Reachability Analyzer (the same tool used in the VPC step):\naws ec2 create-network-insights-path \\\n  --source <APP_SUBNET_ENI_ID> \\\n  --destination <DB_INSTANCE_ENI_ID> \\\n  --protocol tcp \\\n  --destination-port 5432 \\\n  --region <TARGET_REGION>\n\naws ec2 start-network-insights-analysis \\\n  --network-insights-path-id <PATH_ID> \\\n  --region <TARGET_REGION>\n\n# When NACL or SG blocks the path, Reachability Analyzer surfaces the exact rule:\naws ec2 describe-network-insights-analyses \\\n  --network-insights-analysis-ids <ANALYSIS_ID> \\\n  --query "NetworkInsightsAnalyses[].Explanations" --region <TARGET_REGION>',
              sources: [
                { label: 'VPC Security Groups — Best practices (NACL recommendation)', url: 'https://docs.aws.amazon.com/vpc/latest/userguide/vpc-security-groups.html' },
                { label: 'VPC Network ACLs — Limitations', url: 'https://docs.aws.amazon.com/vpc/latest/userguide/vpc-network-acls.html' },
                { label: 'VPC Reachability Analyzer', url: 'https://docs.aws.amazon.com/vpc/latest/reachability/what-is-reachability-analyzer.html' }
              ]
            }
          ],
          refs: [
            ...
            { label: 'VPC Security Groups (NACL recommendation)', url: 'https://docs.aws.amazon.com/vpc/latest/userguide/vpc-security-groups.html' },
            { label: 'VPC Reachability Analyzer', url: 'https://docs.aws.amazon.com/vpc/latest/reachability/what-is-reachability-analyzer.html' }
          ],
          commands: [
            '# ── CAPTURE ──',
      ...
      if (s.networkTopology === 'multi-vpc-tgw') {
        steps.push({
          ...
          description: 'Establish Transit Gateway peering between source and target regions. Configure route propagation and attachment associations. Per AWS, transit gateway route tables enable separation of routing domains: "Use transit gateway route tables to configure routing for your transit gateway attachments... Additionally you can create and manage static routes with a route table. For example, you might have a static route that\'s used as a backup route in the event of a network disruption that affects any dynamic routes."',
          workarounds: [
            {
              title: 'Use separate TGW route tables to isolate prod vs DR vs shared-services',
              pattern: 'AWS Transit Gateway — Route table separation',
              summary: 'Per AWS: "A route table is a table that contains rules that direct how your network traffic is routed between your VPCs and VPNs. Each route in the table contains the range of IP addresses for the destinations that you want to send traffic to." And: "Transit gateway route tables allows you to associate a table with a transit gateway attachment. VPC, VPN, VPN Concentrator, Client VPN, Direct Connect gateway, Peering, and Connect attachments are all supported. When associated, routes for these attachments are propagated from the attachment to the target transit gateway route table. An attachment can be propagated to multiple route tables." Best practice for multi-VPC + DR: keep a dedicated route table per environment (prod/dr/shared) and propagate only the attachments that environment is allowed to reach. This prevents accidental cross-environment routing during cutover and gives you a single switch to flip during planned failover.',
              command: '# Create dedicated route tables in the target region:\naws ec2 create-transit-gateway-route-table \\\n  --transit-gateway-id <TARGET_TGW_ID> \\\n  --tag-specifications "ResourceType=transit-gateway-route-table,Tags=[{Key=Name,Value=prod}]" \\\n  --region <TARGET_REGION>\naws ec2 create-transit-gateway-route-table \\\n  --transit-gateway-id <TARGET_TGW_ID> \\\n  --tag-specifications "ResourceType=transit-gateway-route-table,Tags=[{Key=Name,Value=dr}]" \\\n  --region <TARGET_REGION>\n\n# Associate the cross-region peering attachment with the DR route table only:\naws ec2 associate-transit-gateway-route-table \\\n  --transit-gateway-attachment-id <PEERING_ATTACHMENT_ID> \\\n  --transit-gateway-route-table-id <DR_ROUTE_TABLE_ID> \\\n  --region <TARGET_REGION>\n\n# Add a static backup route per AWS guidance:\naws ec2 create-transit-gateway-route \\\n  --destination-cidr-block <SOURCE_CIDR> \\\n  --transit-gateway-route-table-id <DR_ROUTE_TABLE_ID> \\\n  --transit-gateway-attachment-id <PEERING_ATTACHMENT_ID> \\\n  --region <TARGET_REGION>',
              sources: [
                { label: 'AWS Transit Gateway — Route tables', url: 'https://docs.aws.amazon.com/vpc/latest/tgw/tgw-route-tables.html' }
              ]
            },
            {
              title: 'Verify TGW route propagation per attachment, not just at the gateway level',
              pattern: 'TGW — Route propagation pre-flight',
              summary: 'Per AWS: TGW route tables can have multiple attachments propagating routes into them. The single most common DR-day surprise in a multi-VPC + TGW topology is a peering attachment that exists but has not propagated routes into the DR route table — or that propagated into the wrong table. Verify both the association AND the propagation before trusting cross-region traffic. AWS provides `search-transit-gateway-routes` for the actual data-plane lookup, and `export-transit-gateway-routes` for capturing a baseline you can diff against later.',
              command: '# Verify the peering attachment is associated with the expected route table:\naws ec2 get-transit-gateway-route-table-associations \\\n  --transit-gateway-route-table-id <DR_ROUTE_TABLE_ID> \\\n  --region <TARGET_REGION>\n\n# Verify routes have actually propagated into the route table:\naws ec2 get-transit-gateway-route-table-propagations \\\n  --transit-gateway-route-table-id <DR_ROUTE_TABLE_ID> \\\n  --region <TARGET_REGION>\n\n# Run the actual route lookup that the data plane will use:\naws ec2 search-transit-gateway-routes \\\n  --transit-gateway-route-table-id <DR_ROUTE_TABLE_ID> \\\n  --filters Name=route-search.exact-match,Values=<SOURCE_CIDR> \\\n  --region <TARGET_REGION>\n\n# Optional: export the route tables to S3 for diff-against-known-good baselines:\naws ec2 export-transit-gateway-routes \\\n  --transit-gateway-route-table-id <DR_ROUTE_TABLE_ID> \\\n  --s3-bucket <BACKUP_BUCKET> \\\n  --region <TARGET_REGION>',
              sources: [
                { label: 'AWS Transit Gateway — Route tables', url: 'https://docs.aws.amazon.com/vpc/latest/tgw/tgw-route-tables.html' }
              ]
      ...
      if (s.dataHandling === 'move') {
        ...
        steps.push({
          ...
          workarounds: [
            {
              title: 'Use AWS DMS full-load + CDC for minimal-downtime database migration',
              pattern: 'AWS DMS — full-load-and-cdc migration type',
              summary: 'Per AWS DMS docs: "A task can consist of three major phases: Migration of existing data (Full load); The application of cached changes; Ongoing replication (Change Data Capture)." A full-load-and-cdc task lets you migrate the bulk of the data once, then continue replicating changes until you cut traffic over — limiting source-region downtime to seconds rather than the full migration window. Validate via AWS DMS Premigration Assessments before starting.',
              command: '# Create a task that does full-load THEN ongoing CDC:\naws dms create-replication-task \\\n  --replication-task-identifier <TASK_ID> \\\n  --source-endpoint-arn <SRC_ENDPOINT_ARN> \\\n  --target-endpoint-arn <TGT_ENDPOINT_ARN> \\\n  --replication-instance-arn <REPL_INSTANCE_ARN> \\\n  --migration-type full-load-and-cdc \\\n  --table-mappings file://table-mappings.json \\\n  --replication-task-settings file://task-settings.json \\\n  --region <TARGET_REGION>\n\n# Start the task:\naws dms start-replication-task \\\n  --replication-task-arn <TASK_ARN> \\\n  --start-replication-task-type start-replication \\\n  --region <TARGET_REGION>',
              sources: [
                { label: 'AWS DMS Tasks', url: 'https://docs.aws.amazon.com/dms/latest/userguide/CHAP_Tasks.html' },
                { label: 'DMS Premigration Assessments', url: 'https://docs.aws.amazon.com/dms/latest/userguide/CHAP_Tasks.AssessmentReport.html' }
              ]
            },
            {
              title: 'Enable AWS DMS data validation to verify the migration',
              pattern: 'AWS DMS — Data Validation',
              summary: 'Per AWS: "Data validation is a task setting you can use to have AWS DMS compare the data on your target data store with the data from your source data store." DMS data validation runs alongside the migration and emits CloudWatch metrics for any mismatches. This is the AWS-recommended way to confirm migration integrity instead of ad-hoc row-count or checksum scripts.',
              command: '# Enable validation in the task settings JSON before creating/modifying the task:\n# {\n#   "ValidationSettings": {\n#     "EnableValidation": true,\n#     "ValidationMode": "ROW_LEVEL",\n#     "ThreadCount": 5\n#   }\n# }\n# Then apply via:\naws dms modify-replication-task \\\n  --replication-task-arn <TASK_ARN> \\\n  --replication-task-settings file://task-settings-with-validation.json \\\n  --region <TARGET_REGION>\n\n# Inspect validation status:\naws dms describe-table-statistics \\\n  --replication-task-arn <TASK_ARN> \\\n  --region <TARGET_REGION>',
              sources: [
                { label: 'AWS DMS data validation', url: 'https://docs.aws.amazon.com/dms/latest/userguide/CHAP_Validating.html' }
              ]
            },
            {
              title: 'Use AWS DataSync for non-S3 sources (NFS / SMB / HDFS / on-prem)',
              pattern: 'AWS DataSync — managed file/object transfer',
              summary: 'When migrating from on-prem NFS/SMB/HDFS or another cloud, S3 sync does not apply. Per AWS: "AWS DataSync is a secure, reliable, high-speed file transfer service that helps you quickly and easily transfer your file or object data to, from, and between AWS storage services." DataSync does encryption + integrity validation in-flight and supports cross-region transfers between S3, EFS, FSx, and external storage.',
              command: '# Create source and destination locations, then a task:\naws datasync create-task \\\n  --source-location-arn <SRC_LOC_ARN> \\\n  --destination-location-arn <DST_LOC_ARN> \\\n  --options "VerifyMode=ONLY_FILES_TRANSFERRED,Atime=BEST_EFFORT,Mtime=PRESERVE" \\\n  --region <TARGET_REGION>\n\naws datasync start-task-execution --task-arn <TASK_ARN> --region <TARGET_REGION>',
              sources: [
                { label: 'AWS DataSync', url: 'https://docs.aws.amazon.com/datasync/latest/userguide/what-is-datasync.html' }
              ]
            },
            {
              title: 'Add retry-with-exponential-backoff per AWS Prescriptive Guidance',
              pattern: 'AWS Prescriptive Guidance — Retry with backoff',
              summary: 'Per the AWS Prescriptive Guidance retry-with-backoff pattern, migration scripts that talk to AWS APIs should use exponential backoff with jitter. AWS SDKs include built-in retry logic (configurable max retries via the standard or adaptive retry mode). For the AWS CLI v2, set AWS_RETRY_MODE=adaptive and AWS_MAX_ATTEMPTS to handle throttling automatically during long migrations.',
              command: '# Configure adaptive retry mode for the AWS CLI:\nexport AWS_RETRY_MODE=adaptive\nexport AWS_MAX_ATTEMPTS=10\n\n# Or set per-profile in ~/.aws/config:\n# [profile dr]\n# retry_mode = adaptive\n# max_attempts = 10',
              sources: [
                ...
                { label: 'AWS SDK retry behavior', url: 'https://docs.aws.amazon.com/sdkref/latest/guide/feature-retry-behavior.html' }
              ]
            }
          ],
          refs: [
      ...
      if (s.dataHandling === 'backup-restore') {
        steps.push({
          ...
          workarounds: [
            {
              title: 'Schedule cross-region backup copy via a backup plan, not on-demand',
              pattern: 'AWS Backup — Scheduled cross-region copy',
              summary: 'Per AWS: scheduled cross-region copy is configured inside a backup plan rule (CopyAction with the destination vault ARN), so every backup automatically copies to the target region. On-demand start-copy-job is a one-time operation that does not establish a recurring schedule. For DR, schedule the copy in the plan and monitor copy-job success in CloudWatch Events. Note: AWS Backup does not provide an SLA for copy-job completion times — copy times vary by system load and resource size.',
              command: '# backup-plan.json should include a Rules[].CopyActions[] entry with:\n#   "DestinationBackupVaultArn": "arn:aws:backup:<DR_REGION>:<ACCT>:backup-vault:<DR_VAULT>"\n#   "Lifecycle": { "DeleteAfterDays": 90 }\naws backup create-backup-plan \\\n  --backup-plan file://backup-plan.json \\\n  --region <SOURCE_REGION>',
              sources: [
            ...
            {
              title: 'Add an automated AWS Backup restore-testing plan to validate RTO',
              pattern: 'AWS Backup — Restore Testing',
              summary: 'Per AWS: "Restore testing, a feature offered by AWS Backup, provides automated and periodic evaluation of restore viability, as well as the ability to monitor restore job duration times." This is the AWS-recommended pattern for proving you can hit RTO before an incident. AWS Backup auto-creates restore-test resources, validates them, then deletes them. Use create-restore-testing-plan + create-restore-testing-selection.',
              command: '# 1. Create the testing plan (set frequency, start time, eligible recovery points):\naws backup create-restore-testing-plan \\\n  --restore-testing-plan file://restore-testing-plan.json \\\n  --region <TARGET_REGION>\n\n# 2. Assign protected resources to the plan:\naws backup create-restore-testing-selection \\\n  --restore-testing-plan-name <PLAN_NAME> \\\n  --restore-testing-selection file://selection.json \\\n  --region <TARGET_REGION>\n\n# 3. Inspect results:\naws backup list-restore-jobs --by-restore-testing-plan-arn <PLAN_ARN> --region <TARGET_REGION>',
              sources: [
                { label: 'AWS Backup Restore Testing', url: 'https://docs.aws.amazon.com/aws-backup/latest/devguide/restore-testing.html' },
                { label: 'REL13-BP03 Test DR implementation', url: 'https://docs.aws.amazon.com/wellarchitected/latest/reliability-pillar/rel_planning_for_recovery_dr_tested.html' }
              ]
            },
            {
              title: 'Use a Logically Air-Gapped Vault to harden against ransomware',
              pattern: 'AWS Backup — Logically Air-Gapped Vault',
              summary: 'Per AWS: "A logically air-gapped vault is a specialized vault which provides increased security beyond a standard backup vault, as well as the ability to share vault access to other accounts so that recovery time objectives (RTOs) can be faster and more flexible in case of an incident that requires rapid restoration of resources." Air-gapped vaults are encrypted with an AWS-owned or customer-managed KMS key and are equipped with AWS Backup Vault Lock compliance mode. Use this when DR scope includes ransomware or insider-threat recovery, not just regional failover.',
              command: '# Create a logically air-gapped vault (note: requires recent AWS CLI version):\naws backup create-logically-air-gapped-backup-vault \\\n  --backup-vault-name <AIR_GAPPED_VAULT_NAME> \\\n  --min-retention-days 7 \\\n  --max-retention-days 365 \\\n  --region <TARGET_REGION>\n\n# Then add a copy rule to a backup plan that targets this vault.',
              sources: [
                { label: 'AWS Backup Logically Air-Gapped Vault', url: 'https://docs.aws.amazon.com/aws-backup/latest/devguide/logicallyairgappedvault.html' },
                { label: 'AWS Backup Vault Lock', url: 'https://docs.aws.amazon.com/aws-backup/latest/devguide/vault-lock.html' }
              ]
            ...
            {
              title: 'Confirm AWS Backup does not provide RTO SLAs and plan accordingly',
              pattern: 'AWS Backup — Operational caveat',
              summary: 'Per AWS: "AWS Backup does not provide any service-level agreements (SLAs) for a restore time. Restore times can vary based upon system load and capacity, even for restores containing the same resources." For workloads with strict RTO, do not assume restore-from-backup will hit a fixed time — measure with the restore-testing plan above and add buffer. If the workload requires guaranteed RTO, layer in continuous replication (Aurora Global, DDB Global Tables, or pilot-light architecture) as the primary fast path.',
              sources: [
                { label: 'AWS Backup — Restore behavior', url: 'https://docs.aws.amazon.com/aws-backup/latest/devguide/restoring-a-backup.html' }
              ]
            }
          ],
          refs: [
            ...
            { label: 'AWS Backup — Backup and tag copy', url: 'https://docs.aws.amazon.com/aws-backup/latest/devguide/recov-point-create-a-copy.html' },
            { label: 'AWS Backup Restore Testing', url: 'https://docs.aws.amazon.com/aws-backup/latest/devguide/restore-testing.html' },
            { label: 'Logically Air-Gapped Vault', url: 'https://docs.aws.amazon.com/aws-backup/latest/devguide/logicallyairgappedvault.html' },
            { label: 'Storage migration guide (re:Post)', url: 'https://repost.aws/articles/ARS88PRFUwR4CYk2RqebZVzA' }
          ],
          commands: [
      ...
      steps.push({
        ...
        description: 'SNS and SQS are regional — recreate topics, queues, subscriptions, and DLQ configs in the target region. Update application references to target-region ARNs. Use target-region KMS keys for encrypted queues/topics. Per AWS: an SNS subscription\'s dead-letter queue "must point to an Amazon SQS queue in the same AWS account and Region as your Amazon SNS subscription" — DLQs cannot be shared across regions, so each region\'s integration layer needs its own DLQ.',
        workarounds: [
          {
            title: 'Attach a dead-letter queue to every SNS subscription and every SQS source queue',
            pattern: 'SNS/SQS — Dead-letter queues for resilience',
            summary: 'Per AWS (SNS): "A dead-letter queue is an Amazon SQS queue that an Amazon SNS subscription can target for messages that can\'t be delivered to subscribers successfully." Per AWS (SQS): DLQs let you "isolate unconsumed messages to determine why processing did not succeed." Important constraint per AWS: "The Amazon SNS subscription and Amazon SQS queue must be under the same AWS account and Region" and the SQS DLQ ARN "must point to an Amazon SQS queue in the same AWS account and Region as your Amazon SNS subscription." This means each region\'s integration layer needs its own DLQ — they cannot be shared across regions. AWS recommends "setting the maximum retention period for dead-letter queues to 14 days" and using the `ApproximateNumberOfMessagesVisible` CloudWatch metric for alarms (not `NumberOfMessagesSent`).',
            command: '# 1. Create the DLQ in the target region (regular SQS queue):\naws sqs create-queue \\\n  --queue-name app-events-dlq \\\n  --attributes \'{"MessageRetentionPeriod":"1209600"}\' \\\n  --region <TARGET_REGION>\n\n# 2. Attach DLQ to source SQS queue via redrive policy:\naws sqs set-queue-attributes \\\n  --queue-url <SOURCE_QUEUE_URL> \\\n  --attributes \'{"RedrivePolicy":"{\\"deadLetterTargetArn\\":\\"arn:aws:sqs:<TARGET_REGION>:<ACCT>:app-events-dlq\\",\\"maxReceiveCount\\":\\"5\\"}"}\' \\\n  --region <TARGET_REGION>\n\n# 3. Attach DLQ to SNS subscription via redrive policy:\naws sns set-subscription-attributes \\\n  --subscription-arn <SUBSCRIPTION_ARN> \\\n  --attribute-name RedrivePolicy \\\n  --attribute-value \'{"deadLetterTargetArn":"arn:aws:sqs:<TARGET_REGION>:<ACCT>:app-events-dlq"}\' \\\n  --region <TARGET_REGION>\n\n# 4. Alarm on DLQ depth using ApproximateNumberOfMessagesVisible (NOT NumberOfMessagesSent):\naws cloudwatch put-metric-alarm \\\n  --alarm-name app-events-dlq-depth \\\n  --metric-name ApproximateNumberOfMessagesVisible \\\n  --namespace AWS/SQS \\\n  --statistic Maximum --period 60 --threshold 1 --evaluation-periods 1 \\\n  --comparison-operator GreaterThanOrEqualToThreshold \\\n  --dimensions Name=QueueName,Value=app-events-dlq \\\n  --region <TARGET_REGION>',
            sources: [
              { label: 'SNS Dead-letter queues', url: 'https://docs.aws.amazon.com/sns/latest/dg/sns-dead-letter-queues.html' },
          ...
          {
            title: 'Re-tune redrive policy maxReceiveCount for resilient processing',
            pattern: 'SQS — Redrive policy tuning',
            summary: 'Per AWS: "The `maxReceiveCount` is the number of times a consumer can receive a message from a source queue before it is moved to a dead-letter queue. For example, if the `maxReceiveCount` is set to a low value such as 1, one failure to receive a message would cause the message to move to the dead-letter queue. To ensure that your system is resilient against errors, set the `maxReceiveCount` high enough to allow for sufficient retries." When recreating queues in the target region, do not just copy attributes blindly — review whether the existing maxReceiveCount accounts for the new region\'s endpoint latency and downstream service warm-up after migration. Per AWS: "set the retention period of a dead-letter queue to be longer than the retention period of the original queue."',
            command: '# Inspect the source queue\'s redrive policy first to see existing maxReceiveCount:\naws sqs get-queue-attributes \\\n  --queue-url <SOURCE_QUEUE_URL> \\\n  --attribute-names RedrivePolicy MessageRetentionPeriod \\\n  --region <SOURCE_REGION>\n\n# Apply tuned redrive on the target-region queue (DLQ retention >= source retention):\naws sqs set-queue-attributes \\\n  --queue-url <TARGET_QUEUE_URL> \\\n  --attributes \'{"RedrivePolicy":"{\\"deadLetterTargetArn\\":\\"<DLQ_ARN>\\",\\"maxReceiveCount\\":\\"5\\"}","MessageRetentionPeriod":"345600"}\' \\\n  --region <TARGET_REGION>',
            sources: [
              { label: 'SQS Dead-letter queues — redrive policy', url: 'https://docs.aws.amazon.com/AWSSimpleQueueService/latest/SQSDeveloperGuide/sqs-dead-letter-queues.html' }
            ]
          },
          {
            title: 'Replicate SNS topic-attribute config (delivery policy, FilterPolicy, KMS) — do not assume defaults',
            pattern: 'SNS — Subscription attribute replication',
            summary: 'Per AWS: "Server-side errors can happen when the system responsible for the subscribed endpoint becomes unavailable... For server-side errors caused by AWS managed endpoints backed by Amazon SQS or AWS Lambda, Amazon SNS retries delivery up to 100,015 times, over 23 days." HTTP/SMTP/SMS endpoints get a different default. The source-region subscription likely has custom DeliveryPolicy, FilterPolicy, RawMessageDelivery, or RedrivePolicy attributes that must be carried over manually — `aws sns create-topic`/`subscribe` will only set defaults. Inventory and replay each subscription\'s attributes verbatim, including KMS key ARNs which must be replaced with the target-region KMS key ARN.',
            command: '# Inventory all subscriptions for a topic in the source region:\naws sns list-subscriptions-by-topic --topic-arn <SOURCE_TOPIC_ARN> --region <SOURCE_REGION>\n\n# Get every subscription\'s full attribute set:\nfor SUB in $(aws sns list-subscriptions-by-topic --topic-arn <SOURCE_TOPIC_ARN> \\\n    --query "Subscriptions[].SubscriptionArn" --output text --region <SOURCE_REGION>); do\n  echo "=== $SUB ==="\n  aws sns get-subscription-attributes --subscription-arn $SUB --region <SOURCE_REGION>\ndone\n\n# Replay each non-default attribute on the target-region subscription:\naws sns set-subscription-attributes --subscription-arn <TARGET_SUB_ARN> \\\n  --attribute-name FilterPolicy --attribute-value file://filter-policy.json \\\n  --region <TARGET_REGION>\n\n# Topic-level: replay topic attributes (KmsMasterKeyId must use target-region key):\naws sns set-topic-attributes --topic-arn <TARGET_TOPIC_ARN> \\\n  --attribute-name KmsMasterKeyId --attribute-value <TARGET_REGION_KMS_KEY_ID> \\\n  --region <TARGET_REGION>',
            sources: [
              { label: 'SNS Message delivery retries', url: 'https://docs.aws.amazon.com/sns/latest/dg/sns-message-delivery-retries.html' },
              { label: 'SNS Dead-letter queues', url: 'https://docs.aws.amazon.com/sns/latest/dg/sns-dead-letter-queues.html' }
            ]
          },
          {
            title: 'For FIFO topics, do not use a DLQ if message order matters',
            pattern: 'SQS FIFO + DLQ caveat',
            summary: 'Per AWS: "Don\'t use a dead-letter queue with a FIFO queue if you don\'t want to break the exact order of messages or operations." This is an explicit AWS warning. If your source-region SNS-FIFO + SQS-FIFO pipeline relies on strict ordering (e.g., transactional state changes, edit-decision-list pipelines), pull failed messages back to the source via redrive instead of letting them sit in a DLQ that breaks ordering. Per AWS: "For FIFO queues, the enqueue timestamp resets when the message is moved to a dead-letter queue."',
            command: '# Use the SQS DLQ redrive task to move messages out of a DLQ back to a source queue:\naws sqs start-message-move-task \\\n  --source-arn <DLQ_ARN> \\\n  --destination-arn <ORIGINAL_QUEUE_ARN> \\\n  --max-number-of-messages-per-second 100 \\\n  --region <TARGET_REGION>\n\n# Track in-flight redrive tasks:\naws sqs list-message-move-tasks --source-arn <DLQ_ARN> --region <TARGET_REGION>',
            sources: [
              { label: 'SQS Dead-letter queues — FIFO caveat', url: 'https://docs.aws.amazon.com/AWSSimpleQueueService/latest/SQSDeveloperGuide/sqs-dead-letter-queues.html' }
            ]
          }
        ],
        refs: [
      ...
      steps.push({
        ...
        description: 'AWS WAF web ACLs and Network Firewall rule groups are regional — they must be recreated in the target region. Export existing WAF rules and Network Firewall configurations from the source region, then recreate and associate them with the corresponding resources in the target region. Per AWS WAF guidance, do not enable new ACL rules straight in BLOCK mode in production: "test and tune the rules in count mode with your production traffic before enabling them."',
        workarounds: [
          {
            title: 'Roll out new WAF rules in Count mode first; only switch to Block after tuning',
            pattern: 'AWS WAF — Count-mode rollout',
            summary: 'Per AWS: "We recommend that you test and tune any changes to your AWS WAF protection pack (web ACL) before applying them to your website or web application traffic." And: "Before you deploy your protection pack (web ACL) implementation for production traffic, test and tune it in a staging or testing environment until you are comfortable with the potential impact to your traffic. Then test and tune the rules in count mode with your production traffic before enabling them." Also: "When you create or change a protection pack (web ACL) or other AWS WAF resources, the changes take a small amount of time to propagate to all areas where the resources are stored. The propagation time can be from a few seconds to a number of minutes." After cutover, leave new rules in Count mode for 24–72 hours, review CloudWatch sampled requests, then promote to Block.',
            command: '# 1. Recreate the web ACL in the target region with rules overridden to Count:\n#    Each Rule entry should have "OverrideAction": {"Count": {}} for managed rule groups,\n#    or "Action": {"Count": {}} for custom rules.\naws wafv2 create-web-acl \\\n  --name <WEB_ACL_NAME> --scope REGIONAL \\\n  --default-action Allow={} \\\n  --rules file://waf-rules-count-mode.json \\\n  --visibility-config SampledRequestsEnabled=true,CloudWatchMetricsEnabled=true,MetricName=<METRIC_NAME> \\\n  --region <TARGET_REGION>\n\n# 2. Inspect sampled requests over 24-72h to validate no false positives:\naws wafv2 get-sampled-requests \\\n  --web-acl-arn <TARGET_WEB_ACL_ARN> \\\n  --rule-metric-name <RULE_METRIC_NAME> \\\n  --scope REGIONAL --time-window StartTime=<EPOCH>,EndTime=<EPOCH> \\\n  --max-items 500 --region <TARGET_REGION>\n\n# 3. Once tuning is complete, promote rules from Count to Block by re-applying without OverrideAction:\naws wafv2 update-web-acl \\\n  --name <WEB_ACL_NAME> --scope REGIONAL --id <WEB_ACL_ID> \\\n  --lock-token <LOCK_TOKEN> \\\n  --default-action Allow={} \\\n  --rules file://waf-rules-block-mode.json \\\n  --visibility-config SampledRequestsEnabled=true,CloudWatchMetricsEnabled=true,MetricName=<METRIC_NAME> \\\n  --region <TARGET_REGION>',
            sources: [
              { label: 'AWS WAF — Testing and tuning protections', url: 'https://docs.aws.amazon.com/waf/latest/developerguide/web-acl-testing.html' }
            ]
          },
          {
            title: 'Use AWS Firewall Manager to govern WAF/Network Firewall across regions and accounts',
            pattern: 'AWS Firewall Manager — Centralized policy enforcement',
            summary: 'Per AWS: "AWS Firewall Manager simplifies your administration and maintenance tasks across multiple accounts and resources for a variety of protections, including AWS WAF, AWS Shield Advanced, Amazon VPC security groups and network ACLs, AWS Network Firewall, and Amazon Route 53 Resolver DNS Firewall. With Firewall Manager, you set up your protections just once and the service automatically applies them across your accounts and resources, even as you add new accounts and resources." Use Firewall Manager during migration so the same policy applies in the target region the moment a new resource (ALB, API Gateway, CloudFront) is created — no manual association steps. Per AWS: it "Helps to protect all resources of a particular type" and "Automatically adds protection to resources that are added to your account."',
            command: '# Note: Firewall Manager requires AWS Organizations and an admin account designation.\n# Designate the Firewall Manager admin account (run from the management account):\naws fms associate-admin-account --admin-account <ADMIN_ACCOUNT_ID>\n\n# Create a WAF policy that applies to ALBs in the target region (Firewall Manager admin account):\naws fms put-policy \\\n  --policy file://fms-waf-policy.json \\\n  --region <TARGET_REGION>\n\n# fms-waf-policy.json shape (abbreviated):\n# {\n#   "Policy": {\n#     "PolicyName": "centralized-waf",\n#     "ResourceType": "AWS::ElasticLoadBalancingV2::LoadBalancer",\n#     "ExcludeResourceTags": false,\n#     "RemediationEnabled": true,\n#     "SecurityServicePolicyData": {\n#       "Type": "WAFV2",\n#       "ManagedServiceData": "..."\n#     }\n#   }\n# }',
            sources: [
              { label: 'AWS Firewall Manager', url: 'https://docs.aws.amazon.com/waf/latest/developerguide/fms-chapter.html' }
            ]
          },
          {
            title: 'Re-add AWS Managed Rules rule groups by reference, not by re-export',
            pattern: 'AWS WAF — Managed Rules portability',
            summary: 'Per AWS: "A managed rule group is either an AWS Managed Rules rule group, most of which are free for AWS WAF customers, or an AWS Marketplace managed rule group. You automatically subscribe to the paid AWS Managed Rules rule groups when you add them to your protection pack (web ACL)." AWS Managed Rules and Marketplace rule groups exist independently in every region, so you reference them by Vendor + Name in the target-region web ACL rather than copying their rule contents. This avoids the export-and-rebuild cost for the largest part of most ACLs. Custom rules and IP sets do still need to be recreated.',
            command: '# Inspect the source ACL to see which managed rule groups are referenced:\naws wafv2 get-web-acl --name <WEB_ACL_NAME> --scope REGIONAL --id <ID> --region <SOURCE_REGION> \\\n  --query "WebACL.Rules[].Statement.ManagedRuleGroupStatement"\n\n# In the target-region web-ACL JSON, reference managed rule groups directly by Vendor+Name+Version, e.g.:\n# {\n#   "Name": "AWS-AWSManagedRulesCommonRuleSet",\n#   "Priority": 0,\n#   "OverrideAction": { "Count": {} },\n#   "Statement": {\n#     "ManagedRuleGroupStatement": {\n#       "VendorName": "AWS",\n#       "Name": "AWSManagedRulesCommonRuleSet"\n#     }\n#   },\n#   "VisibilityConfig": { "SampledRequestsEnabled": true, "CloudWatchMetricsEnabled": true, "MetricName": "AWSManagedRulesCommonRuleSetMetric" }\n# }\n\n# Recreate IP sets and regex pattern sets in the target region (these ARE regional):\naws wafv2 create-ip-set --name <SET_NAME> --scope REGIONAL --ip-address-version IPV4 \\\n  --addresses 203.0.113.0/24 198.51.100.0/24 \\\n  --region <TARGET_REGION>',
            sources: [
              { label: 'AWS WAF — Managed Rule Groups', url: 'https://docs.aws.amazon.com/waf/latest/developerguide/waf-rule-statement-type-managed-rule-group.html' }
            ]
          },
          {
            title: 'Stage Network Firewall rule changes via Count-equivalent before turning into a stateful drop',
            pattern: 'Network Firewall — Stateful rule rollout',
            summary: 'Network Firewall stateful rule actions include `aws:alert` (which logs but does not drop) and `aws:drop`. AWS recommends staged rollouts in change-control workflows: deploy stateful rules with `aws:alert` actions first to confirm the rule fires only on intended traffic, review alert logs in CloudWatch Logs or S3, then re-issue the policy with `aws:drop`. This protects against accidental denial of legitimate traffic in the target region after migration. Stateless rule actions are simpler (`aws:pass`/`aws:drop`/`aws:forward_to_sfe`) and should still be reviewed against the source-region production traffic logs before activation.',
            command: '# 1. Update rule group with alert-mode rules first:\naws network-firewall update-rule-group \\\n  --rule-group-arn <RULE_GROUP_ARN> --rule-group file://stateful-rules-alert.json \\\n  --type STATEFUL --region <TARGET_REGION>\n\n# stateful-rules-alert.json should use action "aws:alert" instead of "aws:drop":\n#   "Actions": ["aws:alert"]\n\n# 2. Enable logging so alerts are captured:\naws network-firewall update-logging-configuration \\\n  --firewall-arn <FIREWALL_ARN> \\\n  --logging-configuration \'LogDestinationConfigs=[{LogType=ALERT,LogDestinationType=CloudWatchLogs,LogDestination={logGroup=/aws/network-firewall/alerts}}]\' \\\n  --region <TARGET_REGION>\n\n# 3. After validation window, promote to drop:\naws network-firewall update-rule-group \\\n  --rule-group-arn <RULE_GROUP_ARN> --rule-group file://stateful-rules-drop.json \\\n  --type STATEFUL --region <TARGET_REGION>',
            sources: [
              { label: 'AWS Network Firewall', url: 'https://docs.aws.amazon.com/network-firewall/latest/developerguide/what-is-aws-network-firewall.html' }
            ]
          }
        ],
        refs: [
          ...
          { label: 'AWS WAF — Testing and tuning', url: 'https://docs.aws.amazon.com/waf/latest/developerguide/web-acl-testing.html' },
          { label: 'AWS Firewall Manager', url: 'https://docs.aws.amazon.com/waf/latest/developerguide/fms-chapter.html' },
      ...
      steps.push({
        ...
        description: 'Enable and configure GuardDuty, Cognito user pools, and IAM Access Analyzer in the target region. These services are regional and must be set up independently. For Cognito, user pools cannot be migrated — plan for user re-registration or use a custom migration Lambda trigger. Per AWS: with the Migrate user Lambda trigger "you can seamlessly migrate users from your existing user directory to user pools when a user signs in for the first time with your app or requests a password reset." GuardDuty detectors must be enabled per-region. IAM Access Analyzer must be created per-region to monitor resource policies.',
        workarounds: [
          {
            title: 'Use a Migrate-User Lambda trigger so Cognito users migrate seamlessly on first sign-in',
            pattern: 'Cognito — Just-in-time user migration',
            summary: 'Per AWS: "With this approach, you can seamlessly migrate users from your existing user directory to user pools when a user signs in for the first time with your app or requests a password reset. Add a Migrate user Lambda trigger function to your user pool and it receives metadata about users who try to sign in, and returns user profile information from an external identity source." Important constraint per AWS: "Before you add a user migration trigger, activate the USER_PASSWORD_AUTH or ADMIN_USER_PASSWORD_AUTH flow in the settings of your app client. You must use these flows instead of the default USER_SRP_AUTH flow. Amazon Cognito must send a password to your Lambda function so that it can verify your user\'s authentication in the other directory. An SRP obscures your user\'s password from your Lambda function." After migration completes for a given user, AWS recommends "use USER_SRP_AUTH for sign-in." Critical security note from AWS: "Do not log the entire request event object in your user migration Lambda code. This request event object includes the user\'s password."',
            command: '# 1. Create the Lambda function that authenticates users against the source directory\n#    (source can be the source-region user pool, an external IdP, or a database).\naws lambda create-function \\\n  --function-name cognito-migrate-user-trigger \\\n  --runtime nodejs20.x --handler index.handler --role <ROLE_ARN> \\\n  --zip-file fileb://migrate-handler.zip --region <TARGET_REGION>\n\n# 2. Permit Cognito (and only Cognito + this user pool) to invoke the function:\naws lambda add-permission --function-name cognito-migrate-user-trigger \\\n  --statement-id cognito-allow --action lambda:InvokeFunction \\\n  --principal cognito-idp.amazonaws.com \\\n  --source-arn arn:aws:cognito-idp:<TARGET_REGION>:<ACCOUNT_ID>:userpool/<TARGET_USER_POOL_ID> \\\n  --region <TARGET_REGION>\n\n# 3. Attach the trigger to the target-region user pool:\naws cognito-idp update-user-pool \\\n  --user-pool-id <TARGET_USER_POOL_ID> \\\n  --lambda-config UserMigration=<LAMBDA_ARN> \\\n  --region <TARGET_REGION>\n\n# 4. App client must enable USER_PASSWORD_AUTH (NOT USER_SRP_AUTH) for migration period:\naws cognito-idp update-user-pool-client \\\n  --user-pool-id <TARGET_USER_POOL_ID> --client-id <CLIENT_ID> \\\n  --explicit-auth-flows USER_PASSWORD_AUTH ALLOW_REFRESH_TOKEN_AUTH \\\n  --region <TARGET_REGION>',
            sources: [
              { label: 'Cognito — Migrate user Lambda trigger', url: 'https://docs.aws.amazon.com/cognito/latest/developerguide/cognito-user-pools-import-using-lambda.html' }
            ]
          },
          {
            title: 'Designate one delegated GuardDuty admin per Region — required for AWS Organizations',
            pattern: 'GuardDuty — Delegated administrator across regions',
            summary: 'Per AWS: "Unlike AWS Organizations, GuardDuty is a Regional service. The delegated GuardDuty administrator accounts and their member accounts must be added through AWS Organizations in each desired Region where you have GuardDuty enabled." And: "You must designate one member account as the delegated GuardDuty administrator account across all the AWS Regions where GuardDuty is enabled." When migrating to a new region, the delegated admin must be re-designated in the target region using the same account that is admin elsewhere. AWS notes: "Not recommended to set your organization\'s management account as the delegated GuardDuty administrator account."',
            command: '# Run from the AWS Organizations management account:\naws organizations enable-aws-service-access \\\n  --service-principal guardduty.amazonaws.com\n\n# Designate the delegated GuardDuty admin in the target region:\naws guardduty enable-organization-admin-account \\\n  --admin-account-id <SECURITY_ACCOUNT_ID> \\\n  --region <TARGET_REGION>\n\n# From the delegated admin account, set auto-enable so new and existing org members are protected:\naws guardduty update-organization-configuration \\\n  --detector-id <DETECTOR_ID> \\\n  --auto-enable-organization-members ALL \\\n  --region <TARGET_REGION>',
            sources: [
              { label: 'GuardDuty with AWS Organizations', url: 'https://docs.aws.amazon.com/guardduty/latest/ug/guardduty_organizations.html' }
            ]
          },
          {
            title: 'Pre-stage IAM Access Analyzer with the right scope (account vs organization)',
            pattern: 'IAM Access Analyzer — Region + scope',
            summary: 'IAM Access Analyzer is regional and must be created per-region. There are two analyzer types: account-scoped (`--type ACCOUNT`) detects resource policies that grant access outside the account; organization-scoped (`--type ORGANIZATION`) detects access from outside the AWS Organization. After migration, choose the same scope that the source region used so finding semantics stay consistent. Use archive rules to suppress expected cross-account/cross-region access (e.g., the recovery account intentionally has access during DR test) so production findings stay actionable.',
            command: '# Account-scoped analyzer:\naws accessanalyzer create-analyzer \\\n  --analyzer-name dr-region-analyzer \\\n  --type ACCOUNT \\\n  --region <TARGET_REGION>\n\n# Organization-scoped (run from the delegated admin account):\naws accessanalyzer create-analyzer \\\n  --analyzer-name dr-region-org-analyzer \\\n  --type ORGANIZATION \\\n  --region <TARGET_REGION>\n\n# Archive rule to suppress expected cross-region access patterns:\naws accessanalyzer create-archive-rule \\\n  --analyzer-name dr-region-analyzer \\\n  --rule-name suppress-known-dr-access \\\n  --filter \'{"principal.AWS":{"contains":["<KNOWN_DR_ROLE_ARN>"]}}\' \\\n  --region <TARGET_REGION>',
            sources: [
              { label: 'IAM Access Analyzer overview', url: 'https://docs.aws.amazon.com/IAM/latest/UserGuide/what-is-access-analyzer.html' }
            ]
          },
          {
            title: 'Replicate secrets to the target region with Secrets Manager multi-Region replication',
            pattern: 'Secrets Manager — Multi-region replication',
            summary: 'Per AWS: "You can replicate your secrets in multiple AWS Regions to support applications spread across those Regions to meet Regional access and low latency requirements... Secrets Manager replicates the encrypted secret data and metadata such as tags and resource policies across the specified Regions." Important rotation behavior per AWS: "If you turn on rotation for your primary secret, Secrets Manager rotates the secret in the primary Region, and the new secret value propagates to all of the associated replica secrets. You don\'t have to manage rotation individually for all of the replica secrets." Note from AWS: "If you later need to, you can promote a replica secret to a standalone and then set it up for replication independently" — useful during a planned cutover when the new region becomes the primary.',
            command: '# Add a replica region for an existing secret:\naws secretsmanager replicate-secret-to-regions \\\n  --secret-id app/db-credentials \\\n  --add-replica-regions Region=<TARGET_REGION> \\\n  --region <SOURCE_REGION>\n\n# Or create a new secret with replication baked in:\naws secretsmanager create-secret \\\n  --name app/db-credentials \\\n  --secret-string file://creds.json \\\n  --add-replica-regions Region=<TARGET_REGION> \\\n  --region <SOURCE_REGION>\n\n# When the target region becomes primary, promote the replica:\naws secretsmanager stop-replication-to-replica \\\n  --secret-id app/db-credentials \\\n  --region <TARGET_REGION>',
            sources: [
          ...
          {
            title: 'Replicate SSM Parameter Store config — manual or via shared parameters',
            pattern: 'SSM Parameter Store — Cross-region config replication',
            summary: 'SSM Parameter Store is regional and does NOT have built-in multi-Region replication like Secrets Manager. Per AWS guidance, advanced parameters can be shared across accounts via Resource Access Manager, but cross-Region copy still requires explicit re-creation. For DR, AWS recommends one of: (a) script the export/import as part of your IaC pipeline so source-region parameters are also written to the target region; (b) use AppConfig for dynamic configuration that ships across regions through deployment strategies; (c) for SecureString parameters, ensure target-region KMS keys exist before recreation. Per AWS: "If you manage credentials that require automatic rotation, cross-account access, or fine-grained audit logging, we recommend using AWS Secrets Manager."',
            command: '# Export every parameter under a hierarchy from the source region:\naws ssm get-parameters-by-path \\\n  --path "/app/" --recursive --with-decryption \\\n  --region <SOURCE_REGION> > params.json\n\n# Re-import each into the target region (script the loop, replacing KMS key for SecureString):\nwhile read -r NAME TYPE VALUE; do\n  aws ssm put-parameter \\\n    --name "$NAME" --type "$TYPE" --value "$VALUE" \\\n    --key-id <TARGET_REGION_KMS_KEY_ID> --overwrite \\\n    --region <TARGET_REGION>\ndone < params.tsv\n\n# Verify import succeeded:\naws ssm get-parameters-by-path --path "/app/" --recursive --region <TARGET_REGION> --query "Parameters[].Name"',
            sources: [
        ...
        refs: [
          ...
          { label: 'Secrets Manager — replicate secrets', url: 'https://docs.aws.amazon.com/secretsmanager/latest/userguide/replicate-secrets.html' },
      ...
      if (addSvc.indexOf('fsx') >= 0 && s.dataProfile && s.dataProfile.startsWith('stateful')) {
        steps.push({
          ...
          description: 'FSx file systems are regional. Migration approach depends on file system type: FSx for Windows File Server supports cross-region backup copy (per AWS, "particularly valuable for cross-Region disaster recovery"); FSx for NetApp ONTAP supports volume-level SnapMirror replication ("scheduled as frequently as every 5 minutes"); FSx for OpenZFS supports on-demand cross-region replication; FSx for Lustre can be recreated from its S3 data repository. Inventory existing file systems, plan the migration approach per type, and validate data integrity after restore.' + (_isImpaired(s, 's3') ? ' ⚠ S3 IMPAIRMENT: FSx for Lustre recreation from S3 data repository is not available while S3 is impaired. Use AWS Backup for FSx for Windows, or SnapMirror for ONTAP. Defer Lustre recreation until S3 is restored.' : ''),
          workarounds: [
            {
              title: 'FSx for Windows: schedule cross-region backup copy via AWS Backup, not on-demand',
              pattern: 'FSx for Windows — Cross-region backup copy',
              summary: 'Per AWS: "Cross-Region backup copies are particularly valuable for cross-Region disaster recovery. You take backups and copy them to another AWS Region so that in the event of a disaster in the primary AWS Region, you can restore from backup and recover availability quickly in the other AWS Region." And: "AWS Backup is a unified backup service designed to simplify the creation, copying, restoration, and deletion of backups." Important caveats per AWS: "When you copy a backup to a different destination AWS Region or destination AWS account from the source backup, the first copy is a full backup copy, even if you use the same KMS key... After the first backup copy, all subsequent backup copies to the same destination Region within the same AWS account are incremental, provided that you haven\'t deleted all previously-copied backups in that Region and have been using the same AWS KMS key." Also: "You can have up to five backup copy requests in progress to a single destination AWS Region per account." Restore caveat: "You can only restore a file system backup to a new file system with the same deployment type and storage capacity as the original."',
              command: '# AWS-recommended: schedule via a backup plan rule rather than ad-hoc start-copy-job.\n# backup-plan.json must include Rules[].CopyActions[] with the destination vault ARN.\naws backup create-backup-plan \\\n  --backup-plan file://fsx-windows-backup-plan.json \\\n  --region <SOURCE_REGION>\n\n# Assign the FSx file systems to the plan via a resource selection:\naws backup create-backup-selection \\\n  --backup-plan-id <PLAN_ID> \\\n  --backup-selection file://fsx-resource-selection.json \\\n  --region <SOURCE_REGION>\n\n# Verify cross-region copy job success:\naws backup list-copy-jobs --by-state COMPLETED \\\n  --by-resource-type FSx --region <TARGET_REGION>',
              sources: [
                { label: 'FSx for Windows — Backups and cross-region copy', url: 'https://docs.aws.amazon.com/fsx/latest/WindowsGuide/using-backups.html' },
                { label: 'AWS Backup Cross-Region copy', url: 'https://docs.aws.amazon.com/aws-backup/latest/devguide/cross-region-backup.html' }
              ]
            },
            {
              title: 'FSx for NetApp ONTAP: schedule volume-level SnapMirror replication for low RPO',
              pattern: 'FSx for ONTAP — SnapMirror scheduled replication',
              summary: 'Per AWS: "You can use NetApp SnapMirror to schedule periodic replication of your FSx for ONTAP file system to or from a second file system. This capability is available for both in-Region and cross-Region deployments." And: "NetApp SnapMirror replicates data at high speeds, so you get high data availability and fast data replication across ONTAP systems... Replication can be scheduled as frequently as every 5 minutes, although intervals should be carefully chosen based on RPOs (Recovery Point Objectives), RTOs (Recovery Time Objectives), and performance considerations." Important constraint per AWS: "There are two types of SnapMirror replication: Volume-level SnapMirror and SVM Disaster Recovery (SVMDR). Only volume-level SnapMirror replication is supported by FSx for ONTAP. Synchronous SnapMirror, including StrictSync, is not supported." That means the AWS-supported floor is asynchronous, volume-scoped — application-level consistency across volumes is your responsibility.',
              command: '# 1. Create the destination data-protection (DP) volume on the target-region FSx file system:\naws fsx create-volume \\\n  --file-system-id <TARGET_FILE_SYSTEM_ID> \\\n  --name dr-vol-replica \\\n  --volume-type ONTAP \\\n  --ontap-configuration \'{"OntapVolumeType":"DP","JunctionPath":"/dr-vol-replica","SizeInMegabytes":102400,"StorageEfficiencyEnabled":true,"StorageVirtualMachineId":"<SVM_ID>"}\' \\\n  --region <TARGET_REGION>\n\n# 2. Schedule replication from ONTAP CLI / NetApp Console (AWS does not expose snapmirror schedule via CLI):\n#    Connect via the SVM management endpoint; example NetApp ONTAP CLI:\n#      cluster::> snapmirror create -source-path <SRC_SVM>:<SRC_VOL> \\\n#                 -destination-path <DST_SVM>:<DST_VOL> \\\n#                 -schedule 5min -policy MirrorAllSnapshots\n#      cluster::> snapmirror initialize -destination-path <DST_SVM>:<DST_VOL>\n\n# 3. Validate replication health regularly via the NetApp ONTAP CLI:\n#      cluster::> snapmirror show -fields state,lag-time,exported-snapshot',
              sources: [
                { label: 'FSx for ONTAP — Scheduled replication using SnapMirror', url: 'https://docs.aws.amazon.com/fsx/latest/ONTAPGuide/scheduled-replication.html' }
              ]
            },
            {
              title: 'FSx for OpenZFS: use on-demand snapshot replication across regions',
              pattern: 'FSx for OpenZFS — On-demand replication',
              summary: 'Per AWS: "Amazon FSx for OpenZFS supports on-demand data replication, enabling you to transfer snapshots of data between file systems within and across AWS Regions and accounts." Replication is suitable for "Maintaining a passive standby file system for use in disaster recovery cases." Per AWS: "Each file system can only be used as the source file system or the destination file system for one on-demand data replication task at a time. You must wait until the first on-demand replication task is completed or cancelled before initiating another request. You can only have a maximum of twenty concurrent cross-file system replication jobs per account, per AWS Region." Performance note from AWS: "On-demand data replication shares provisioned throughput with other file system clients. To accommodate data replication activity without impacting other workloads, we recommend provisioning twice the level of throughput capacity that your workload normally needs."',
              command: '# Create destination volume in target region (or pre-create one):\naws fsx create-volume \\\n  --volume-type OPENZFS \\\n  --name dr-vol \\\n  --open-zfs-configuration \'{"ParentVolumeId":"<TARGET_PARENT_VOL>","StorageCapacityReservationGiB":100,"DataCompressionType":"ZSTD"}\' \\\n  --region <TARGET_REGION>\n\n# Trigger an on-demand incremental copy from a source snapshot ARN:\naws fsx copy-snapshot-and-update-volume \\\n  --volume-id <DESTINATION_VOLUME_ID> \\\n  --source-snapshot-arn <SOURCE_SNAPSHOT_ARN> \\\n  --copy-strategy INCREMENTAL_COPY \\\n  --options DELETE_INTERMEDIATE_SNAPSHOTS DELETE_CLONED_VOLUMES DELETE_INTERMEDIATE_DATA \\\n  --region <TARGET_REGION>\n\n# Track progress (ProgressPercent + RemainingTransferBytes appear in AdministrativeActions):\naws fsx describe-volumes --volume-ids <DESTINATION_VOLUME_ID> --region <TARGET_REGION>',
              sources: [
                { label: 'FSx for OpenZFS — On-demand replication', url: 'https://docs.aws.amazon.com/fsx/latest/OpenZFSGuide/on-demand-replication.html' }
              ]
            },
            {
              title: 'FSx for Windows: pre-validate Active Directory and storage parity before restore',
              pattern: 'FSx for Windows — Restore guardrails',
              summary: 'Per AWS: "To ensure that users can continue to access the restored file system, make sure that the Active Directory domain associated with the restored file system is the same as that of the original file system, or is trusted by the Active Directory domain of the original file system." And: "You can only restore a file system backup to a new file system with the same deployment type and storage capacity as the original. You can increase the new file system\'s storage capacity after it becomes available." After migration these constraints commonly cause failed restores; pre-validate AD trust and the deployment type/storage size BEFORE the cutover so the restore does not fail when you need it most.',
              command: '# Pre-flight check: confirm the target-region AD trust is in place:\naws ds describe-trusts --directory-id <TARGET_DIRECTORY_ID> --region <TARGET_REGION>\n\n# Pre-flight check: capture source FS deployment type + storage capacity:\naws fsx describe-file-systems --file-system-ids <SOURCE_FS_ID> \\\n  --query "FileSystems[].{Type:WindowsConfiguration.DeploymentType,GiB:StorageCapacity,SubnetIds:SubnetIds}" \\\n  --region <SOURCE_REGION>\n\n# Restore from a backup with matching attributes:\naws fsx create-file-system-from-backup \\\n  --backup-id <TARGET_REGION_BACKUP_ID> \\\n  --subnet-ids <SUBNET_ID> \\\n  --windows-configuration ActiveDirectoryId=<TARGET_DIRECTORY_ID>,ThroughputCapacity=32 \\\n  --region <TARGET_REGION>',
              sources: [
                { label: 'FSx for Windows — Restoring backups', url: 'https://docs.aws.amazon.com/fsx/latest/WindowsGuide/using-backups.html' }
              ]
            }
          ],
          refs: [
            ...
            { label: 'FSx for ONTAP — Scheduled replication', url: 'https://docs.aws.amazon.com/fsx/latest/ONTAPGuide/scheduled-replication.html' },
            { label: 'FSx for OpenZFS — On-demand replication', url: 'https://docs.aws.amazon.com/fsx/latest/OpenZFSGuide/on-demand-replication.html' },
      ...
      if (s.appType === 'ec2' || s.appType === 'mixed') {
        steps.push({
          ...
          workarounds: [
            {
              title: 'Use Data Lifecycle Manager (DLM) to automate snapshots and cross-region copy',
              pattern: 'Amazon Data Lifecycle Manager — Snapshot policies',
              summary: 'Per AWS EBS docs: snapshots are point-in-time copies, and "for data resiliency and disaster recovery, it is your responsibility to create EBS snapshots on a regular basis, or to set up automatic snapshot creation by using Automate data backups with AWS Backup and Amazon Data Lifecycle Manager." DLM can take and cross-region-copy snapshots on a schedule with retention rules — eliminating ad-hoc copy steps during recovery. Use this for any volume holding state worth recovering.',
              command: 'aws dlm create-lifecycle-policy \\\n  --execution-role-arn <DLM_ROLE_ARN> \\\n  --description "DR snapshot + cross-region copy policy" \\\n  --state ENABLED \\\n  --policy-details file://dlm-policy.json \\\n  --region <SOURCE_REGION>\n\n# dlm-policy.json should include a Schedules[].CrossRegionCopyRules[]\n# entry with TargetRegion=<TARGET_REGION>, Encrypted=true, CmkArn=<TARGET_KMS_KEY>.',
              sources: [
                { label: 'EBS Snapshots', url: 'https://docs.aws.amazon.com/ebs/latest/userguide/EBSSnapshots.html' },
                { label: 'EBS Snapshot Lifecycle Manager', url: 'https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/snapshot-lifecycle.html' }
              ]
            },
            {
              title: 'Cross-region copies are full the first time, incremental thereafter — keep at least one copy alive',
              pattern: 'Amazon EBS — Incremental snapshot copying',
              summary: 'Per AWS: "When you copy a snapshot to a new Region, a full (non-incremental) copy is created." However, "Snapshot copy operations within the same account and Region using the same KMS key are always incremental copies." For cross-region, subsequent copies become incremental IF "the most recent snapshot copy still exists in the destination Region or account" AND "all copies of the snapshot in the destination Region or account are either unencrypted or were encrypted using the same KMS key." So: keep at least one prior copy in the destination region — deleting all of them forces the next copy to be a full copy, increasing data transfer cost and time.',
              command: '# List existing snapshot copies already in the destination region:\naws ec2 describe-snapshots \\\n  --owner-ids self \\\n  --region <TARGET_REGION> \\\n  --filters "Name=volume-id,Values=<SOURCE_VOLUME_ID>"\n\n# Always retain at least the most recent destination-region copy before deletion lifecycles fire.',
              sources: [
                { label: 'EBS Copy Snapshot — Incremental copying', url: 'https://docs.aws.amazon.com/ebs/latest/userguide/ebs-copy-snapshot.html' }
              ]
            },
            {
              title: 'Enable EBS Encryption by Default in the destination region for compliance',
              pattern: 'Amazon EBS — Encryption by default',
              summary: 'Per AWS EBS docs: when "Encryption by default" is enabled in the destination region, snapshot copies are automatically encrypted regardless of source encryption status. This guarantees that no unencrypted snapshot enters the recovery region. Configure this once per region; future copies inherit it.',
              command: '# Enable encryption by default in the target region:\naws ec2 enable-ebs-encryption-by-default --region <TARGET_REGION>\n\n# Set the default KMS key (otherwise aws/ebs is used):\naws ec2 modify-ebs-default-kms-key-id \\\n  --kms-key-id <TARGET_KMS_KEY_ARN> \\\n  --region <TARGET_REGION>\n\n# Verify:\naws ec2 get-ebs-encryption-by-default --region <TARGET_REGION>',
              sources: [
                { label: 'EBS Encryption by default', url: 'https://docs.aws.amazon.com/ebs/latest/userguide/encryption-by-default.html' }
              ]
            },
            {
              title: 'For very large volumes, consider time-based copies for predictable completion',
              pattern: 'Amazon EBS — Time-based snapshot copies',
              summary: 'Per AWS EBS docs, the copy-snapshot command supports a "Time-based copies" mode where you "enter the required completion duration, in 15-minute increments." Use this when you need predictable copy completion within a maintenance window — without it, copies are best-effort and can take hours depending on size and source-region load.',
              command: 'aws ec2 copy-snapshot \\\n  --source-region <SOURCE_REGION> \\\n  --source-snapshot-id <SNAP_ID> \\\n  --destination-region <TARGET_REGION> \\\n  --kms-key-id <TARGET_KMS_KEY> \\\n  --encrypted \\\n  --completion-duration-minutes 15 \\\n  --description "Time-based DR copy" \\\n  --region <TARGET_REGION>',
              sources: [
                { label: 'EBS Copy Snapshot', url: 'https://docs.aws.amazon.com/ebs/latest/userguide/ebs-copy-snapshot.html' }
              ]
            }
      ...
      if (!isPanic) {
        steps.push({
          ...
          description: 'Use AWS Resilience Hub to assess the resilience posture of your application in the target region. Per AWS: "AWS Resilience Hub is a central location for you to manage and improve the resilience posture of your applications on AWS. AWS Resilience Hub enables you to define your resilience goals, assess your resilience posture against those goals, and implement recommendations for improvement based on the AWS Well-Architected Framework." Run this BEFORE cutover to catch RTO/RPO gaps; re-run on a schedule afterward to detect configuration drift.',
          workarounds: [
            {
              title: 'Pair the Resilience Hub assessment with AWS FIS chaos experiments — AWS-recommended',
              pattern: 'Resilience Hub + AWS Fault Injection Service (FIS)',
              summary: 'Per AWS: "Within AWS Resilience Hub, you can also create and run AWS Fault Injection Service experiments, which mimic real-life disruptions to your application to help you better understand dependencies and uncover potential weaknesses." Per AWS FIS docs: "AWS Fault Injection Service (AWS FIS) is a managed service that enables you to perform fault injection experiments on your AWS workloads. Fault injection is based on the principles of chaos engineering. These experiments stress an application by creating disruptive events so that you can observe how your application responds." Important AWS warning: "AWS FIS carries out real actions on real AWS resources in your system. Therefore, before you use AWS FIS to run experiments in production, we strongly recommend that you complete a planning phase and run the experiments in a pre-production environment." For DR, run FIS experiments that match the failure modes you actually expect: AZ failure, RDS failover, dependency latency injection.',
              command: '# 1. Inspect FIS experiment templates Resilience Hub generates from its assessment:\naws resiliencehub list-app-component-recommendations \\\n  --assessment-arn <ASSESSMENT_ARN> \\\n  --region <TARGET_REGION>\n\n# 2. Run a non-destructive AZ-failure experiment in a pre-prod copy first:\naws fis create-experiment-template \\\n  --description "AZ-failure simulation matching DR scope" \\\n  --role-arn <FIS_ROLE_ARN> \\\n  --actions \'{"az-power-interruption":{"actionId":"aws:network:disrupt-connectivity","parameters":{"duration":"PT5M","scope":"availability-zone"},"targets":{"Subnets":"target-az-subnets"}}}\' \\\n  --targets \'{"target-az-subnets":{"resourceType":"aws:ec2:subnet","selectionMode":"ALL","filters":[{"path":"AvailabilityZone","values":["<AZ_ID>"]}]}}\' \\\n  --stop-conditions \'[{"source":"aws:cloudwatch:alarm","value":"<HEALTH_ALARM_ARN>"}]\' \\\n  --region <PRE_PROD_REGION>\n\naws fis start-experiment --experiment-template-id <TEMPLATE_ID> --region <PRE_PROD_REGION>',
              sources: [
                { label: 'AWS Resilience Hub overview', url: 'https://docs.aws.amazon.com/resilience-hub/latest/userguide/what-is.html' },
                { label: 'AWS Fault Injection Service overview', url: 'https://docs.aws.amazon.com/fis/latest/userguide/what-is.html' }
              ]
            },
            {
            ...
            {
              title: 'Set the resiliency policy from REL13-BP01-defined RTO/RPO, not from convenience',
              pattern: 'REL13-BP01 — RTO/RPO grounded in business impact',
              summary: 'Per REL13-BP01: "Failures can impact your business in several ways. First, failures can cause service interruption (downtime). Second, failures can cause data to become lost, inconsistent, or stale. In order to guide how you respond and recover from failures, define a Recovery Time Objective (RTO) and Recovery Point Objective (RPO) for each workload." AWS warns about common anti-patterns: "You select arbitrary recovery objectives," "You select recovery objectives that are too lenient and do not meet business objectives," and "You select unrealistic recovery objectives, such as zero time to recover or zero data loss, which may not be achievable for your workload configuration." Make sure the RTO/RPO numbers in your Resilience Hub policy match the business-impact tier the workload was assigned, not the numbers that happen to fit your current architecture.',
              command: '# Inspect the current policy and the assessment that ran against it:\naws resiliencehub describe-resiliency-policy \\\n  --policy-arn <POLICY_ARN> --region <TARGET_REGION>\n\naws resiliencehub describe-app-assessment \\\n  --assessment-arn <ASSESSMENT_ARN> --region <TARGET_REGION>\n\n# Update the resiliency policy if business stakeholders set tighter targets:\naws resiliencehub update-resiliency-policy \\\n  --policy-arn <POLICY_ARN> \\\n  --policy \'{"Region":{"rpoInSecs":900,"rtoInSecs":3600},"AZ":{"rpoInSecs":0,"rtoInSecs":0},"Hardware":{"rpoInSecs":0,"rtoInSecs":0},"Software":{"rpoInSecs":0,"rtoInSecs":0}}\' \\\n  --tier MissionCritical \\\n  --region <TARGET_REGION>',
              sources: [
                { label: 'REL13-BP01 Recovery objectives', url: 'https://docs.aws.amazon.com/wellarchitected/latest/reliability-pillar/rel_planning_for_recovery_objective_defined_recovery.html' }
              ]
            }
          ...
          refs: [
            ...
            { label: 'REL13-BP01 Recovery objectives', url: 'https://docs.aws.amazon.com/wellarchitected/latest/reliability-pillar/rel_planning_for_recovery_objective_defined_recovery.html' },
            { label: 'REL13-BP03 Test DR', url: 'https://docs.aws.amazon.com/wellarchitected/latest/reliability-pillar/rel_planning_for_recovery_dr_tested.html' },
            { label: 'AWS Fault Injection Service', url: 'https://docs.aws.amazon.com/fis/latest/userguide/what-is.html' }
          ],
          commands: [
      ...
      steps.push({
        ...
        workarounds: [
          {
            title: 'Run the DR drill on the path you intend to use — partial drills mask real gaps',
            pattern: 'REL13-BP03 — Exercise the actual recovery path',
            summary: 'Per REL13-BP03 (verbatim): "A pattern to avoid is developing recovery paths that are rarely exercised. For example, you might have a secondary data store that is used for read-only queries. When you write to a data store and the primary fails, you might want to fail over to the secondary data store. If you don\'t frequently test this failover, you might find that your assumptions about the capabilities of the secondary data store are incorrect. The capacity of the secondary, which might have been sufficient when you last tested, might be no longer be able to tolerate the load under this scenario. Our experience has shown that the only error recovery that works is the path you test frequently. This is why having a small number of recovery paths is best." Anti-pattern AWS calls out explicitly: "Never exercise failovers in production." For DR-grade drills, AWS recommends a small number of well-exercised paths over many barely-tested ones.',
            command: '# 1. Drive synthetic load against the target region while traffic is still served from source.\n#    Stand up a CloudWatch Synthetics canary that mirrors a real customer journey:\naws synthetics create-canary \\\n  --name dr-drill-canary \\\n  --code S3Bucket=<BUCKET>,S3Key=canary.zip,Handler=index.handler \\\n  --artifact-s3-location s3://<ARTIFACT_BUCKET>/dr-drill/ \\\n  --execution-role-arn <CANARY_ROLE_ARN> \\\n  --schedule Expression="rate(1 minute)" \\\n  --runtime-version syn-nodejs-puppeteer-9.0 \\\n  --region <TARGET_REGION>\n\n# 2. Force a temporary DNS shift to the target region for a measurable interval.\n#    Time-box the drill (e.g. 30 min) and monitor the canary + ALB metrics:\naws route53 change-resource-record-sets \\\n  --hosted-zone-id <ZONE_ID> \\\n  --change-batch file://drill-failover.json\n\n# 3. After the drill, revert and CAPTURE the measurements (RTO actuals, error rate, latency p99).\n#    Document them in a runbook artifact so the same drill can be repeated.',
            sources: [
              { label: 'REL13-BP03 Test DR', url: 'https://docs.aws.amazon.com/wellarchitected/latest/reliability-pillar/rel_planning_for_recovery_dr_tested.html' },
              { label: 'CloudWatch Synthetics canaries', url: 'https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/CloudWatch_Synthetics_Canaries.html' }
            ]
          ...
          {
            title: 'Use AWS Elastic Disaster Recovery (DRS) drill instances for non-disruptive failover testing',
            pattern: 'AWS Elastic DR — Drill instances',
            summary: 'Per REL13-BP03: "For Amazon EC2-based workloads, use AWS Elastic Disaster Recovery to implement and launch drill instances for your DR strategy. AWS Elastic Disaster Recovery provides the ability to efficiently run drills, which helps you prepare for a failover event. You can also frequently launch of your instances using Elastic Disaster Recovery for test and drill purposes without redirecting the traffic." This is the AWS-supported way to test the actual recovery launch path without disrupting production. Drill instances run alongside your replication, so the next real failover does not face untested code paths.',
            command: '# Identify protected source servers in DRS:\naws drs describe-source-servers --region <TARGET_REGION>\n\n# Launch a non-disruptive recovery drill (creates instances in target region without redirecting traffic):\naws drs start-recovery \\\n  --is-drill \\\n  --source-servers \'[{"sourceServerID":"<SOURCE_SERVER_ID>","recoverySnapshotID":"<SNAPSHOT_ID>"}]\' \\\n  --region <TARGET_REGION>\n\n# Verify drill launched and capture timings:\naws drs describe-job-log-items --job-id <JOB_ID> --region <TARGET_REGION>\n\n# After validation, terminate the drill instances:\naws drs terminate-recovery-instances \\\n  --recovery-instance-ids <INSTANCE_ID> \\\n  --region <TARGET_REGION>',
            sources: [
              { label: 'REL13-BP03 (DRS recommendation)', url: 'https://docs.aws.amazon.com/wellarchitected/latest/reliability-pillar/rel_planning_for_recovery_dr_tested.html' }
            ]
          },
          {
            title: 'Inject realistic failures with AWS FIS in pre-prod before any production cutover',
            pattern: 'AWS FIS — Pre-prod chaos validation',
            summary: 'Per AWS FIS: "AWS FIS provides templates that generate disruptions, and the controls and guardrails that you need to run experiments in production, such as automatically rolling back or stopping the experiment if specific conditions are met." Important AWS warning: "AWS FIS carries out real actions on real AWS resources in your system. Therefore, before you use AWS FIS to run experiments in production, we strongly recommend that you complete a planning phase and run the experiments in a pre-production environment." For pre-cutover validation, run AZ disruption, RDS failover, and dependency latency injection in pre-prod that mirrors the new target-region topology. Define stop-conditions tied to your CloudWatch alarms so any cascade is automatically halted.',
            command: '# Create an experiment template with stop-conditions wired to a CloudWatch alarm:\naws fis create-experiment-template \\\n  --description "Pre-cutover RDS failover drill" \\\n  --role-arn <FIS_ROLE_ARN> \\\n  --stop-conditions \'[{"source":"aws:cloudwatch:alarm","value":"<ERROR_RATE_ALARM_ARN>"}]\' \\\n  --actions \'{"failover-db":{"actionId":"aws:rds:failover-db-cluster","parameters":{},"targets":{"Clusters":"target-cluster"}}}\' \\\n  --targets \'{"target-cluster":{"resourceType":"aws:rds:cluster","resourceArns":["<RDS_CLUSTER_ARN>"],"selectionMode":"ALL"}}\' \\\n  --region <PRE_PROD_REGION>\n\n# Start the experiment in pre-prod:\naws fis start-experiment --experiment-template-id <TEMPLATE_ID> --region <PRE_PROD_REGION>\n\n# Watch progress and stop conditions:\naws fis get-experiment --id <EXPERIMENT_ID> --region <PRE_PROD_REGION>',
            sources: [
              { label: 'AWS Fault Injection Service overview', url: 'https://docs.aws.amazon.com/fis/latest/userguide/what-is.html' }
            ]
          }
      ...
      steps.push({
        ...
        description: isPanic ? 'Execute immediate DNS cutover to target region. Monitor closely for 24 hours. Accept elevated risk due to reduced testing window.' : 'Execute production cutover during the approved maintenance window. Switch DNS to target region. Monitor for 48-72 hours before declaring success. Per AWS DR architecture guidance: "Make sure you set TTL (time to live) on these records low enough so that DNS resolvers will reflect your changes quickly enough to meet your RTO targets" — the most common cutover surprise is DNS TTL longer than the RTO budget.',
        workarounds: [
          {
            title: 'Lower DNS TTL well before the cutover, not at the cutover moment',
            pattern: 'Route 53 — Pre-cutover TTL reduction',
            summary: 'Per AWS architecture guidance: "Make sure you set TTL (time to live) on these records low enough so that DNS resolvers will reflect your changes quickly enough to meet your RTO targets." The trap: if your records have a 1-hour TTL today and you change the value at cutover, downstream resolvers will keep the old answer for up to that hour. AWS-recommended pattern: lower the TTL to 60 seconds at least 24 hours before the planned cutover, so caches everywhere have already converged to the short TTL. After cutover stability is confirmed, raise the TTL back to its operational baseline (5–60 min) to reduce DNS query cost and load.',
            command: '# Pre-cutover (T-24h or earlier): lower TTL to 60 on every record involved in the cutover.\n# Use UPSERT to change ONLY the TTL — do not modify Name/Type or downstream impact.\naws route53 change-resource-record-sets \\\n  --hosted-zone-id <ZONE_ID> \\\n  --change-batch \'{\n    "Changes":[{\n      "Action":"UPSERT",\n      "ResourceRecordSet":{\n        "Name":"<HOSTNAME>","Type":"A",\n        "TTL":60,\n        "ResourceRecords":[{"Value":"<CURRENT_IP>"}]\n      }\n    }]\n  }\'\n\n# Verify the new TTL has propagated to public resolvers (takes up to the OLD TTL value):\ndig <HOSTNAME> +noall +answer @8.8.8.8\ndig <HOSTNAME> +noall +answer @1.1.1.1\n\n# After cutover stability (T+48-72h), restore TTL to operational baseline (e.g. 300s):\naws route53 change-resource-record-sets \\\n  --hosted-zone-id <ZONE_ID> \\\n  --change-batch \'{\n    "Changes":[{"Action":"UPSERT","ResourceRecordSet":{"Name":"<HOSTNAME>","Type":"A","TTL":300,"AliasTarget":{...}}}]\n  }\'',
            sources: [
              { label: 'AWS DR Architecture — Multi-site Active/Active', url: 'https://aws.amazon.com/blogs/architecture/disaster-recovery-dr-architecture-on-aws-part-iv-multi-site-active-active/' }
            ]
          },
          {
          ...
          {
            title: 'Drive a synthetic canary against the target region during cutover for end-to-end visibility',
            pattern: 'CloudWatch Synthetics — Cutover validation canary',
            summary: 'Per AWS: "Canaries follow the same routes and perform the same actions as a customer, which makes it possible for you to continually verify your customer experience even when you don\'t have any customer traffic on your applications. By using canaries, you can discover issues before your customers do." Per AWS: canaries "can run as often as once per minute." During the cutover window, a 1-minute canary against the target region\'s public hostname becomes the authoritative end-to-end signal — separate from ALB metrics or application logs — that the customer journey is actually working post-DNS-shift. If the canary fails, that\'s your trigger to roll back, not the absence of a 5xx alarm.',
            command: '# 1. Create a 1-minute canary BEFORE cutover so a baseline exists:\naws synthetics create-canary \\\n  --name cutover-customer-journey \\\n  --code S3Bucket=<BUCKET>,S3Key=customer-journey.zip,Handler=index.handler \\\n  --artifact-s3-location s3://<ARTIFACT_BUCKET>/canary-artifacts/ \\\n  --execution-role-arn <CANARY_ROLE_ARN> \\\n  --schedule Expression="rate(1 minute)" \\\n  --runtime-version syn-nodejs-puppeteer-9.0 \\\n  --start-canary \\\n  --region <TARGET_REGION>\n\n# 2. During cutover, alarm on canary failures:\naws cloudwatch put-metric-alarm \\\n  --alarm-name cutover-canary-failure \\\n  --metric-name Failed \\\n  --namespace CloudWatchSynthetics \\\n  --dimensions Name=CanaryName,Value=cutover-customer-journey \\\n  --statistic Sum --period 60 --threshold 1 --evaluation-periods 1 \\\n  --comparison-operator GreaterThanOrEqualToThreshold \\\n  --alarm-actions <SNS_INCIDENT_TOPIC_ARN> \\\n  --region <TARGET_REGION>',
            sources: [
              { label: 'CloudWatch Synthetics canaries', url: 'https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/CloudWatch_Synthetics_Canaries.html' }
            ]
          }
        ...
        refs: [
          { label: 'Route 53 health checks', url: 'https://docs.aws.amazon.com/Route53/latest/DeveloperGuide/welcome-health-checks.html' },
          { label: 'CloudWatch Synthetics canaries', url: 'https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/CloudWatch_Synthetics_Canaries.html' },
          { label: 'AWS DR Architecture — Multi-site Active/Active', url: 'https://aws.amazon.com/blogs/architecture/disaster-recovery-dr-architecture-on-aws-part-iv-multi-site-active-active/' }
        ],
        commands: [
      ...
      steps.push({
        ...
        description: 'Structured validation to confirm data integrity, application health, and operational readiness before declaring migration successful. Do NOT decommission source region resources or finalize migration until all validation checks pass. Per REL13-BP03: a recovery plan is "not done until you have proved it works" — that proof is the structured validation evidence captured here, not the absence of immediate alarms.',
        workarounds: [
          {
            title: 'Re-run AWS Backup Restore Testing in the new region BEFORE decommissioning source',
            pattern: 'AWS Backup — Post-cutover restore proof',
            summary: 'Per AWS: "Restore testing, a feature offered by AWS Backup, provides automated and periodic evaluation of restore viability, as well as the ability to monitor restore job duration times." After cutover, the source-region backup posture is no longer the production posture — what matters is whether the new (target-region) backup chain meets RTO. Re-run the restore-testing plan against the new vault and capture the durations; only then is the new RTO claim defensible. Source-region resources should not be decommissioned until the new region has demonstrated a successful end-to-end restore at least once.',
            command: '# Verify the target-region restore-testing plan exists and is active:\naws backup list-restore-testing-plans --region <TARGET_REGION>\n\n# Run an on-demand restore test using the existing plan (or wait for the scheduled run):\naws backup start-restore-job \\\n  --recovery-point-arn <TARGET_REGION_RECOVERY_POINT> \\\n  --metadata file://restore-metadata.json \\\n  --iam-role-arn <RESTORE_ROLE_ARN> \\\n  --region <TARGET_REGION>\n\n# Inspect measured restore durations against your declared RTO:\naws backup list-restore-jobs \\\n  --by-state COMPLETED \\\n  --query "sort_by(RestoreJobs,&CreationDate)[-10:].{ARN:RestoreJobId,Resource:ResourceType,Status:Status,CreatedAt:CreationDate,FinishedAt:CompletionDate}" \\\n  --region <TARGET_REGION>',
            sources: [
              { label: 'AWS Backup — Restore Testing', url: 'https://docs.aws.amazon.com/aws-backup/latest/devguide/restore-testing.html' },
              { label: 'REL13-BP03 Test DR', url: 'https://docs.aws.amazon.com/wellarchitected/latest/reliability-pillar/rel_planning_for_recovery_dr_tested.html' }
            ]
          ...
          {
            title: 'Verify the new region\'s recovery objectives still hold using a fresh Resilience Hub assessment',
            pattern: 'Resilience Hub — Post-cutover compliance proof',
            summary: 'Per AWS: "AWS Resilience Hub enables you to define your resilience goals, assess your resilience posture against those goals, and implement recommendations for improvement based on the AWS Well-Architected Framework." Re-run the Resilience Hub assessment against the post-cutover topology and confirm the compliance status is still meeting the policy. The cutover may have introduced new resources (DR-specific Lambda functions, Route 53 health checks, expanded DDB Global Tables) that change the resilience surface — the pre-cutover assessment alone does not cover them.',
            command: '# Publish a fresh Resilience Hub app version reflecting the post-cutover topology:\naws resiliencehub publish-app-version \\\n  --app-arn <APP_ARN> \\\n  --region <TARGET_REGION>\n\n# Run an assessment against the new published version:\naws resiliencehub start-app-assessment \\\n  --app-arn <APP_ARN> --app-version published \\\n  --assessment-name post-cutover-validation \\\n  --region <TARGET_REGION>\n\n# Wait until the assessment completes, then inspect compliance:\naws resiliencehub describe-app-assessment \\\n  --assessment-arn <ASSESSMENT_ARN> \\\n  --query "Assessment.{Status:AssessmentStatus,Compliance:ComplianceStatus,RTO:Compliance,Recommendations:ResiliencyScore}" \\\n  --region <TARGET_REGION>\n\naws resiliencehub list-app-component-compliances \\\n  --assessment-arn <ASSESSMENT_ARN> \\\n  --region <TARGET_REGION>',
            sources: [
              { label: 'AWS Resilience Hub overview', url: 'https://docs.aws.amazon.com/resilience-hub/latest/userguide/what-is.html' }
            ]
          },
          ...
          {
            title: 'Do not decommission source region resources until 14+ days of post-cutover stability',
            pattern: 'AWS DR — Conservative source-region retention',
            summary: 'Per the AWS DR whitepaper, the source region remains a viable rollback path until the target region has demonstrated stability under real production traffic. Cutover-day green metrics are not equivalent to "no regression" — many bugs (capacity, third-party integration timeouts, scheduled batch jobs, end-of-month workflows) only surface days or weeks later. AWS-recommended posture: keep the source region in standby for at least one full operational cycle (typically 2 weeks for transactional workloads, longer for monthly-batch workloads) before decommissioning. During the retention window, keep replication running so a back-cutover is still possible.',
            command: '# Catalog the source-region resources you would decommission so the list is auditable:\naws resourcegroupstaggingapi get-resources \\\n  --tag-filters Key=Migration,Values=source-region \\\n  --region <SOURCE_REGION> \\\n  --query "ResourceTagMappingList[].ResourceARN" --output table > source-decom-candidates.txt\n\n# Confirm replication is still flowing (cross-region replicas, DDB Global Tables, S3 RTC, etc.):\naws rds describe-db-instances --region <SOURCE_REGION> \\\n  --query "DBInstances[?ReadReplicaSourceDBInstanceIdentifier!=null]"\naws s3api list-buckets --query "Buckets[].Name" \\\n  | xargs -I {} aws s3api get-bucket-replication --bucket {} 2>/dev/null\n\n# Only AFTER the retention window AND a successful back-cutover drill, schedule decommissioning:\n# Document the decommission list in a runbook so the action is auditable and reversible until execution.',
            sources: [
              { label: 'AWS DR whitepaper', url: 'https://docs.aws.amazon.com/whitepapers/latest/disaster-recovery-workloads-on-aws/disaster-recovery-options-in-the-cloud.html' }
            ]
          }
      ...
      if (s.backupTechnology === 'aws-backup') {
        steps.push({
          ...
          workarounds: [
            {
              title: 'Use GetRecoveryPointRestoreMetadata to get the right restore parameters',
              pattern: 'AWS Backup — Restore metadata',
              summary: 'Per AWS: "The configuration values (\\"restore metadata\\") that you need to restore your resource varies depending on the resource that you want to restore. To get the configuration metadata that your backup was created with, you can call GetRecoveryPointRestoreMetadata." This is the AWS-recommended way to populate restore-metadata.json instead of guessing — particularly important during incidents when getting the parameters wrong wastes an entire restore-job cycle (typically minutes to hours).',
              command: 'aws backup get-recovery-point-restore-metadata \\\n  --backup-vault-name <VAULT_NAME> \\\n  --recovery-point-arn <RECOVERY_POINT_ARN> \\\n  --region <TARGET_REGION>\n\n# Use the returned RestoreMetadata as the basis for your start-restore-job --metadata file.',
              sources: [
                { label: 'AWS Backup — How to restore', url: 'https://docs.aws.amazon.com/aws-backup/latest/devguide/restoring-a-backup.html' }
              ]
            },
            {
              title: 'AWS Backup creates a NEW resource per restore — do not assume in-place overwrite',
              pattern: 'AWS Backup — Non-destructive restore',
              summary: 'Per AWS: "When you use AWS Backup to restore a backup, it creates a new resource with the backup that you are restoring. This is to protect your existing resources from being destroyed by your restore activity." Important caveats: Amazon Redshift Serverless namespace restores AND SAP HANA on EC2 perform DESTRUCTIVE restores that overwrite existing data. For all other resources, plan to redirect application traffic to the new resource (e.g., update DNS, RDS Proxy, parameter store entries) — the restored resource has a new ARN and endpoint.',
              command: '# Start the restore job (returns RestoreJobId):\naws backup start-restore-job \\\n  --recovery-point-arn <RECOVERY_POINT_ARN> \\\n  --metadata file://restore-metadata.json \\\n  --iam-role-arn <RESTORE_ROLE_ARN> \\\n  --region <TARGET_REGION>\n\n# Monitor (status: PENDING, RUNNING, COMPLETED, ABORTED, FAILED):\naws backup describe-restore-job \\\n  --restore-job-id <RESTORE_JOB_ID> \\\n  --region <TARGET_REGION>',
              sources: [
                { label: 'AWS Backup — Non-destructive restores', url: 'https://docs.aws.amazon.com/aws-backup/latest/devguide/restoring-a-backup.html' }
              ]
            },
            {
              title: 'Set CopySourceTagsToRestoredResource=true to preserve tags during restore',
              pattern: 'AWS Backup — Tag-on-restore',
              summary: 'Per AWS: "In the API operation StartRestoreJob, the parameter CopySourceTagsToRestoredResource is set to false by default, which will exclude the original source tags from the resource you are restoring. If you wish to include tags from the original source, set this to True." This matters when IAM policies restrict access by tag — without source tags, the restored resource may be inaccessible to the same principals that owned the source. EC2 / EFS / RDS support tag-on-restore; DynamoDB / S3 / SAP HANA / Timestream / VMware do NOT support it (per AWS).',
              command: '# Include CopySourceTagsToRestoredResource in the start-restore-job request:\naws backup start-restore-job \\\n  --recovery-point-arn <RECOVERY_POINT_ARN> \\\n  --metadata file://restore-metadata.json \\\n  --iam-role-arn <RESTORE_ROLE_ARN> \\\n  --copy-source-tags-to-restored-resource \\\n  --region <TARGET_REGION>',
              sources: [
                { label: 'AWS Backup — Tag-on-restore', url: 'https://docs.aws.amazon.com/aws-backup/latest/devguide/restoring-a-backup.html' }
              ]
            },
            {
              title: 'Cold-storage recovery points take ~4 extra hours — plan around it',
              pattern: 'AWS Backup — Cold storage retrieval',
              summary: 'Per AWS: "Restoring from cold storage typically takes 4 hours more than restoring from warm storage." If your backup plan transitioned recovery points to cold storage (Glacier-tier), factor that retrieval delay into your RTO. For tier-0 / tier-1 workloads, keep recent recovery points in warm storage; only age out to cold storage beyond your maximum RTO window.',
              command: '# Check the storage class of a recovery point:\naws backup describe-recovery-point \\\n  --backup-vault-name <VAULT_NAME> \\\n  --recovery-point-arn <RECOVERY_POINT_ARN> \\\n  --region <TARGET_REGION> \\\n  --query "{Status:Status,StorageClass:StorageClass,LastRestoreTime:LastRestoreTime}"',
              sources: [
                { label: 'AWS Backup — Restore time and cold storage', url: 'https://docs.aws.amazon.com/aws-backup/latest/devguide/restoring-a-backup.html' }
              ]
            }
          ],
          refs: [
      ...
      if (needsCrossRegionCopy && !_isImpaired(s, 's3')) {
        steps.push({
          ...
          workarounds: [
            {
              title: 'Automate snapshots and cross-region copy via AWS Backup or DLM (avoid one-off copy commands)',
              pattern: 'AWS Backup / DLM — Scheduled cross-region copy',
              summary: 'Per AWS Backup docs, scheduled cross-region copy via a backup plan is the AWS-recommended pattern. For pure-EBS workloads, Data Lifecycle Manager (DLM) provides the same automation natively. The advantage over ad-hoc copy-snapshot commands during an incident: the copies already exist when you need them, restore-from-existing is materially faster than copy-then-restore, and you avoid copy-job queueing that AWS notes can take "several hours to complete" depending on size and load.',
              command: 'aws backup create-backup-plan \\\n  --backup-plan file://backup-plan-with-copy-action.json \\\n  --region <SOURCE_REGION>\n\n# OR for pure-EBS via DLM:\naws dlm create-lifecycle-policy \\\n  --execution-role-arn <DLM_ROLE_ARN> \\\n  --description "Snapshot + cross-region copy" \\\n  --state ENABLED \\\n  --policy-details file://dlm-policy.json \\\n  --region <SOURCE_REGION>',
              sources: [
                { label: 'AWS Backup — Backup and tag copy', url: 'https://docs.aws.amazon.com/aws-backup/latest/devguide/recov-point-create-a-copy.html' },
                { label: 'EBS Data Lifecycle Manager', url: 'https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/snapshot-lifecycle.html' }
              ]
            },
            {
              title: 'Use multi-Region KMS keys to skip the cross-region re-encryption step',
              pattern: 'AWS KMS multi-Region keys — DR pattern',
              summary: 'Per AWS: "you can encrypt data in one AWS Region and decrypt it in a different AWS Region without re-encrypting or making a cross-Region call to AWS KMS." When a snapshot is encrypted with a multi-Region KMS key (and the replica key exists in the target region), the cross-region copy doesn\'t need to re-encrypt — significantly faster, fewer KMS calls, no cross-region KMS dependency during the copy.',
              command: '# Verify KMS key is multi-Region:\naws kms describe-key \\\n  --key-id <KMS_KEY_ID> \\\n  --region <SOURCE_REGION> \\\n  --query "KeyMetadata.{MultiRegion:MultiRegion,KeyId:KeyId}"\n\n# If MultiRegion=true, the same key ID exists in the target region as a replica key.',
              sources: [
                { label: 'KMS Multi-Region Keys', url: 'https://docs.aws.amazon.com/kms/latest/developerguide/multi-region-keys-overview.html' }
              ]
            },
            {
              title: 'Use AWS Backup copy-job retry behavior — system errors auto-retry for 2 hours',
              pattern: 'AWS Backup — Copy job retry strategy',
              summary: 'Per AWS: "If AWS Backup encounters any system errors, the copy job enters a retry phase that lasts for 2 hours. During this time, the copy job status remains in the CREATED state while the system periodically attempts to initiate the job. If the job successfully starts within this window, it transitions to the RUNNING state." This means a transient cross-region network issue does not require manual retry — AWS Backup handles it. For ec2 copy-snapshot / rds copy-db-snapshot (EC2/RDS APIs, not AWS Backup), retry-with-backoff is your responsibility — wrap calls in client-side retry logic.',
              command: 'aws backup describe-copy-job \\\n  --copy-job-id <COPY_JOB_ID> \\\n  --region <TARGET_REGION>\n\n# Status values: CREATED (in retry phase), RUNNING, COMPLETED, FAILED, ABORTED',
              sources: [
                { label: 'AWS Backup — Copy job retry', url: 'https://docs.aws.amazon.com/aws-backup/latest/devguide/recov-point-create-a-copy.html' }
              ]
            },
            {
              title: 'Concurrency limits: only one backup or copy job per resource at a time',
              pattern: 'AWS Backup — Copy job concurrency',
              summary: 'Per AWS: "Only one backup or copy job can run at a time for a given resource. Additional copy jobs for the same resource remain in CREATED status until the running job completes." For ec2 copy-snapshot, AWS imposes a separate "limit of 20 concurrent snapshot copy requests per destination" per region. Plan around these limits when designing recovery — staggered copy jobs are slower than parallel copies of different resources.',
              command: '# List queued copy jobs to understand current concurrency:\naws backup list-copy-jobs \\\n  --by-state CREATED \\\n  --region <TARGET_REGION>\n\n# For EC2 snapshot copies, monitor the queue depth via:\naws ec2 describe-snapshots \\\n  --owner-ids self \\\n  --filters "Name=status,Values=pending" \\\n  --region <TARGET_REGION>',
              sources: [
                { label: 'AWS Backup — Copy job concurrency', url: 'https://docs.aws.amazon.com/aws-backup/latest/devguide/recov-point-create-a-copy.html' },
                { label: 'EBS Copy Snapshot — Concurrency', url: 'https://docs.aws.amazon.com/ebs/latest/userguide/ebs-copy-snapshot.html' }
              ]
            }
          ],
          refs: [
            ...
            { label: 'AWS Backup copy job behavior', url: 'https://docs.aws.amazon.com/aws-backup/latest/devguide/recov-point-create-a-copy.html' },
            { label: 'EBS Data Lifecycle Manager', url: 'https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/snapshot-lifecycle.html' },
            { label: 'Multi-Region KMS keys', url: 'https://docs.aws.amazon.com/kms/latest/developerguide/multi-region-keys-overview.html' }
          ],
          commands: [
  ...
  function renderRunbookTab(runbook, isPanic) {
    ...
    runbook.forEach(function (step, i) {
      ...
      if (step.commands && step.commands.length) { h += '<div class="runbook-step__section"><div class="runbook-step__section-title">📜 Step Commands (used by the bash runbook export)</div><div style="font-size:11px;color:var(--ts);margin:-4px 0 6px 0">These are the canonical commands bundled when you click "Download Runbook as Bash" — the workarounds above include their own AWS-grounded commands per pattern.</div><div class="code-block"><div class="code-block__header"><span>bash</span><button class="code-block__copy" data-copy-target="rb-' + i + '">Copy</button></div><pre id="rb-' + i + '"><code>'; step.commands.forEach(function (c) { h += esc(c) + '\n'; }); h += '</code></pre></div></div>'; }
