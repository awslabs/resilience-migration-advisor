 ...
 * - IMMEDIATE_DR: edit immediate DR mode runbook (inside getRunbookSteps)
 *
 * NEW DIMENSIONS (v2):
 ...
 * - urgencyMode: architecture-strategy | immediate-dr
 */
...
(function () {
    ...
    rules: [
      {
        ...
        partner: 'n2ws',
        confidence: 'High',
        reason: 'Immediate DR with heavy databases requires rapid backup and recovery — N2W specializes in automated backup, snapshot management, and fast restore for database-heavy workloads.'
      },
      {
        id: 'r2',
  ...
  var WIZARD_STEPS = [
    {
      ...
      columns: 2,
      options: [
        { value: 'self-execution', label: 'I want to execute recovery myself', description: 'Use guided architecture strategy or immediate DR tools to plan and execute recovery on your own.', weight: 0, icon: '🛠️' },
    ...
    },
    {
      ...
      description: 'Architecture Strategy optimizes for best architecture. Immediate DR prioritizes fastest possible recovery. Regional Partner Assistance connects you with MENA-region AWS partners. Partner Matchmaking recommends the best-fit partner for your situation.',
      columns: 2,
      getOptions: function (s) {
        ...
        return [
          ...
          { value: 'immediate-dr', label: 'Immediate DR', description: 'Fastest possible recovery. Prioritize speed over optimization. For active incidents or imminent threats.', weight: 30, icon: '🚨' }
        ];
      },
      options: [
        { value: 'architecture-strategy', label: 'Architecture Strategy', description: 'Optimize architecture based on RTO/RPO targets and best practices.', weight: 0, icon: '🎯' },
    ...
    },
    {
      ...
      description: 'Each partner offers a different approach to rapid disaster recovery. Select one to get tailored immediate execution steps.',
      columns: 3,
    ...
    },
    {
      ...
      description: 'Urgency determines whether you need immediate DR tools or planned migration.',
      conditional: function (s) { return s.proceedPath === 'partner-assisted' && s.urgencyMode === 'matchmaking'; },
      columns: 3,
      options: [
        { value: 'immediate', label: 'Immediate DR', description: 'Active incident. Need recovery now.', weight: 0, icon: '🚨' },
  ...
  var RULES_ENGINE = {
    getArchitecture: function (s) {
      if (s.urgencyMode === 'immediate-dr') return 'backup-restore'; // immediate-dr = fastest viable
    ...
    getRisks: function (s) {
      ...
      if (s.urgencyMode === 'immediate-dr') {
        r.unshift('IMMEDIATE DR: Elevated risk of data loss — RPO may not be met due to replication lag');
        r.unshift('IMMEDIATE DR: Reduced testing window increases failure risk — validate critical paths first');
    ...
    getRunbookSteps: function (s) {
      ...
      // IMMEDIATE DR: Step — Freeze & Capture
      if (isPanic) {
        steps.push({
          title: 'IMMEDIATE DR: Freeze Changes & Capture Current State',
      ...
      if (s.dataHandling === 'move') {
        steps.push({
          ...
          description: 'One-time data migration using AWS DMS for databases and S3 sync for object storage. Validate data integrity after transfer. Include retry with exponential backoff in migration scripts to handle API throttling. AWS SDKs include built-in retry logic — configure max retries appropriately.',
          refs: [
          ...
          commands: [
            ...
            'aws dms create-replication-task --replication-task-identifier <TASK_ID> --source-endpoint-arn <SRC_ARN> --target-endpoint-arn <TGT_ARN> --migration-type full-load --replication-instance-arn <INST_ARN>'
          ],
          validation: ['S3 object counts match', 'Database row counts match', 'Data integrity checksums verified'],
      ...
      if (s.dataHandling === 'replicate') {
        steps.push({
          ...
          description: 'Set up continuous cross-region replication for S3, RDS read replicas, and DynamoDB Global Tables. Monitor replication lag. Include retry with exponential backoff in automation scripts.',
          refs: [
            { label: 'Retry with backoff pattern', url: 'https://docs.aws.amazon.com/prescriptive-guidance/latest/cloud-design-patterns/retry-backoff.html' },
          ...
          commands: [
            ...
            'aws rds create-db-instance-read-replica --db-instance-identifier <REPLICA_ID> --source-db-instance-identifier <SOURCE_DB> --region <TARGET_REGION>',
            'aws dynamodb update-table --table-name <TABLE> --replica-updates "Create={RegionName=<TARGET_REGION>}"'
          ],
          validation: ['S3 replication status active', 'RDS replica lag < threshold', 'DynamoDB replica in sync'],
      ...
      if (s.dataHandling === 'backup-restore') {
        steps.push({
          ...
          description: 'Set up AWS Backup plans for periodic cross-region backup copies. Test restore procedures in target region.',
          refs: [
            { label: 'Storage migration guide (re:Post)', url: 'https://repost.aws/articles/ARS88PRFUwR4CYk2RqebZVzA' },
      ...
      if (addSvc.indexOf('fsx') >= 0 && s.dataProfile && s.dataProfile.startsWith('stateful')) {
        steps.push({
          ...
          commands: [
            ...
            '# Ensure S3 data is replicated to target region first, then create new Lustre FS',
  ...
  function renderStep() {
    ...
    if (isPanic && currentStep > 0) html += '<div class="callout callout--warning" style="margin-bottom:16px"><strong>🚨 IMMEDIATE DR ACTIVE</strong> — Prioritizing fastest recovery path.</div>';
  ...
  function renderPartnerEngagementGuide() {
      ...
      var h = '';
      h += '<div class="callout callout--info" style="margin-bottom:20px"><strong>\uD83E\uDD1D Partner Engagement Workflow</strong><br>Follow the steps below to engage your selected partner for disaster recovery or migration assistance. Steps 1\u20134 are customer actions; steps 5+ are executed by the partner.</div>';
      ...
      // ── Partners WITHOUT marketplace → contact info + basic discovery only ──
        ...
        h += '<p style="font-size:13px;color:var(--tl);margin-bottom:8px">Run these commands to document your current environment before engaging the partner. This will accelerate the initial consultation.</p>';
      ...
      if (partnerInfo || drToolInfo) {
        ...
        h += '<li style="list-style:disc;margin-bottom:4px">Validate data residency and compliance requirements before engaging partner in cross-region operations.</li>';
  ...
  function renderMatchmakingResult(recommendation) {
    // Hide tabs — show single panel like Immediate DR
    document.getElementById('results-tabs').style.display = 'none';
    document.getElementById('sidebar-results-links').style.display = 'none';
  ...
  function showResults() {
    ...
    var isPanic = state.urgencyMode === 'immediate-dr';
    badge.textContent = isPanic ? '\uD83D\uDEA8 IMMEDIATE DR' : 'Results';
    ...
    // ============================================================
    // IMMEDIATE DR — Partner-only results
    // ============================================================
    if (isPanic) {
      ...
      // KPI
      var kpi = document.getElementById('kpi-grid');
      kpi.innerHTML = kpiCard('MODE', 'IMMEDIATE DR', 'red', 'Immediate Recovery') +
      ...
      panel.classList.add('results-panel--active');
      var h = '';
      h += '<div class="callout callout--warning" style="margin-bottom:20px"><strong>\uD83D\uDEA8 IMMEDIATE DR — Immediate DR Execution Plan</strong><br>Follow the steps below in order. Each step includes commands you can copy and execute immediately.</div>';
      // Health-aware region advisory
      h += renderHealthRegionAdvisory();
    ...
    if (isPanic) kpi.innerHTML += kpiCard('MODE', 'IMMEDIATE DR', 'red', 'Immediate DR');
  ...
  function renderSummaryTab(arch, archLabels, complexity, risk, actions, isPanic) {
    ...
    var h = '';
    if (isPanic) h += '<div class="callout callout--warning"><strong>🚨 IMMEDIATE DR</strong> — This plan prioritizes fastest recovery. Elevated risk of data loss. Validate compliance requirements.</div>';
    // Health-aware region advisory
    h += renderHealthRegionAdvisory();
    // Summary grid
  ...
  function renderRunbookTab(runbook, isPanic) {
    ...
    if (isPanic) {
      ...
      if (partnerInfo) {
        h += '<div class="result-card" style="margin-top:16px;border-left:3px solid var(--or)"><div class="result-card__header"><span class="result-card__title">\uD83E\uDD1D ' + esc(partnerInfo.fullName) + ' — Immediate DR Steps</span>';
  ...
  function renderTraceTab(isPanic) {
    ...
    if (isPanic) h += '<div class="badge badge--red" style="margin-bottom:12px"><span class="badge__dot"></span>IMMEDIATE DR</div>';
  ...
  function renderRisksTab(risks, isPanic) {
    ...
    risks.forEach(function (r) { var icon = r.startsWith('IMMEDIATE DR') ? '🚨' : '\u26A0'; h += '<div class="checklist__item"><span class="checklist__icon" style="color:var(--or)">' + icon + '</span><span>' + escLink(r) + '</span></div>'; });
    h += '</div>';
    ...
    if (isPanic) h += '<div class="callout callout--warning" style="margin-top:8px"><strong>⚠ Immediate DR Warning:</strong> Reduced testing window. RPO may not be met. Compliance validation may be incomplete. Data loss risk acknowledged.</div>';
  ...
  function generateMarkdown() {
    ...
    // ============================================================
    // FIX #4: Immediate DR — Partner-only markdown (no strategy runbook)
    // ============================================================
    if (state.urgencyMode === 'immediate-dr') {
      var partnerStep = WIZARD_STEPS.find(function (s) { return s.id === 'panic-partner'; });
      var selectedPartner = state.panicPartner;
      var partnerInfo = partnerStep && partnerStep.partnerDetails && selectedPartner ? partnerStep.partnerDetails[selectedPartner] : null;
      var md = '# AWS RMA — Immediate DR Execution Plan\n\nGenerated: ' + new Date().toISOString().split('T')[0] + '\n\n';
      md += '> **🚨 IMMEDIATE DR** — Fastest recovery prioritized.\n\n';
      md += '## Mode: Immediate DR\n\n';
      if (partnerInfo) {
    ...
    // Architecture Strategy (only — Immediate DR handled above)
    ...
    if (isPanic) md += '\n> **\ud83d\udea8 IMMEDIATE DR** \u2014 Fastest recovery prioritized.\n';
  ...
  function copySummary() {
    ...
    // FIX #4: Immediate DR — partner-only summary
    else if (state.urgencyMode === 'immediate-dr') {
      var partnerStep = WIZARD_STEPS.find(function (s) { return s.id === 'panic-partner'; });
      var selectedPartner = state.panicPartner;
      var partnerInfo = partnerStep && partnerStep.partnerDetails && selectedPartner ? partnerStep.partnerDetails[selectedPartner] : null;
      var lines = ['AWS RMA — Immediate DR Summary', ''];
      lines.push('Mode: Immediate DR');
      if (partnerInfo) {
  ...
  var CORS_PROXIES = [
    // allorigins — JSON wrapper (most reliable public CORS proxy)
    ...
    // corsproxy.io — direct passthrough
    function (url) { return { url: 'https://corsproxy.io/?' + encodeURIComponent(url), extract: function (d) { return d; }, name: 'corsproxy' }; }
  ];
